...
##            print('Path followed: %d,%d' % (grid.x, grid.y))
            ...
            if true_grid.blocked and return_grid is self.start:
######                print('Blocked at: %d,%d' % (grid.x, grid.y))
                return_grid = last_grid
                true_grid.path = True
                break
            else:
                true_grid.path = True
##            print(grid.blocked)
        ...
        tgrid = grid
        while tgrid is not None and tgrid.parent is not self.start:
...
##            tgrid.path = False
    ...
    def display_grid(self):
        pygame.display.set_caption("Repeated Backward A* Implementation")
        screen.fill(BLACK)
        done = False
        while done == False:
...
######            print('Starting at (x,y): %d,%d' % (self.start.x, self.start.y))
            self.update_partial_maze(true_maze, self.end)
            self.run_AStar()
            grid_reached = self.check_probable_path(true_maze)

            if grid_reached is self.start:
##                print('Done is True')
                done = True
            else:
##                print('Done is False')
