...
##        dg = self.get_heuristic(exp)
        exp.parent = grid
        print('Counter , g ---> %d,%d' % (self.counter, exp.g))
##        grid.next = exp
##        exp.f = dg + exp.g
##        self.cost = self.counter+self.get_heuristic(exp)

    def run_AStar(self):
        try:
            heapq.heappush(self.open, (self.start.f, self.start.g, self.start))
     ##       heapq.heappush(self.open, ((self.counter + self.get_heuristic(self.start)), self.start))
            while len(self.open):
                f, g, grid = heapq.heappop(self.open)
##                if len(self.open):
##                    tgrids = heapq.nsmallest(3,self.open)
##                    for tg in tgrids:
##                        key, ttg = tg
##                        if (f == key):
##                            if (f-self.get_heuristic(grid) < key-self.get_heuristic(ttg)):
##                                heapq.heappush(self.open, (f, grid))
##                                f= key
##                                grid = ttg
    ##            grid.visited = True
                self.close.add(grid)
                ...
                for exp_grid in exp_grids:
                    if not exp_grid.blocked and exp_grid not in self.close:
                        ...
                        heapq.heappush(self.open, (exp_grid.g+self.get_heuristic(exp_grid),exp_grid.g, exp_grid))
##                        if (exp_grid.f, exp_grid) in self.open:
####                            if exp_grid.g > grid.g + 10:
##                            if exp_grid.g > grid.g+1:
##                                self.update_grid(exp_grid, grid)
##                        else:
##                            self.update_grid(exp_grid, grid)
