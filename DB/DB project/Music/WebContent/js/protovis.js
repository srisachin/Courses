 ...
 *      documentation.
 * @param {function}
 *            f function that produces an element of the new Array from an
 *            element of the current one.
 * @param [o]
 *            object to use as <tt>this</tt> when executing <tt>f</tt>.
 */
 ...
 *      documentation.
 * @param {function}
 *            f function to test each element of the array.
 * @param [o]
 *            object to use as <tt>this</tt> when executing <tt>f</tt>.
 */
 ...
 *      documentation.
 * @param {function}
 *            f function to execute for each element.
 * @param [o]
 *            object to use as <tt>this</tt> when executing <tt>f</tt>.
 */
 ...
 *      documentation.
 * @param {function}
 *            f function to execute on each value in the array.
 * @param [v]
...
/**
 * Returns the passed-in argument, <tt>x</tt>; the identity function. This
 * method is provided for convenience since it is used as the default behavior
 * for a number of property functions.
 * 
 * @param x
 *            a value.
...
/**
 * Returns <tt>this.index</tt>. This method is provided for convenience for
 * use with scales. For example, to color bars by their index, say:
 * 
 * <pre>
...
/**
 * Returns <tt>this.parent.index</tt>. This method is provided for
 * convenience for use with scales. This method is provided for convenience for
 * use with scales. For example, to color bars by their parent index, say:
 * 
 * <pre>
 * .fillStyle(pv.Colors.category10().by(pv.parent))
 * </pre>
 * 
 * Tthis method is equivalent to <tt>function() this.parent.index</tt>, but
 * more succinct.
 * 
 * @see pv.Scale
 * @see pv.Mark#index
 ...
 *          <tt>f</tt>. Rather than constructing a new instance of <tt>f</tt>
 *          to serve as the prototype (which unnecessarily runs the constructor
 *          on the created prototype object, potentially polluting it), an
 *          anonymous function is generated internally that shares the same
 *          prototype:
 * 
 * <pre>
 * function g() {
 ...
 *          functions such that the code can be run by a JavaScript 1.6
 *          interpreter. This hack only supports function expressions (using
 *          clumsy regular expressions, no less), and not other JavaScript 1.8
 *          features such as let expressions.
 * 
 * @param {string}
 *            s a Protovis specification (i.e., a string of JavaScript 1.8
 *            source code).
 ...
 * 
 * @param {string}
 *            p the name of the CSS property.
 * @param e
 *            the element on which to compute the CSS property.
 */
pv.css = function(e, p) {
 ...
 *          the specified target. For standards-compliant browsers, this method
 *          uses <tt>addEventListener</tt>; for Internet Explorer,
 *          <tt>attachEvent</tt>.
 * 
 * @param target
 *            a DOM element.
 ...
 *          {@link pv.event} is set for the duration of the listener's
 *          invocation. The wrapper is cached on the returned function, such
 *          that duplicate registrations of the wrapped event handler are
 *          ignored.
 * 
 * @param {function}
 *            f an event handler.
...
/**
 * @private Returns true iff <i>a</i> is an ancestor of <i>e</i>. This is
 *          useful for ignoring mouseout and mouseover events that are contained
 ...
 * 
 * @class Represents an abstract text formatter and parser. A <i>format</i> is
 *        a function that converts an object of a given type, such as a
 *        <tt>Date</tt>, to a human-readable string representation. The
 *        format may also have a {@link #parse} method for converting a string
 *        representation back to the given object type.
 * 
 * <p>
 * Because formats are themselves functions, they can be used directly as mark
 * properties. For example, if the data associated with a label are dates, a
 * date format can be used as label text:
 * 
 * <pre>
 ...
 *          this methods returns an appropriately quoted version of the
 *          specified string, with any special characters escaped.
 * 
 * @param {string}
 *            s a string to quote.
...
/**
 * @private Optionally pads the specified string <i>s</i> so that it is at
 *          least <i>n</i> characters long, using the padding character <i>c</i>.
 * 
 * @param {string}
 *            c the padding character.
 * @param {number}
 *            n the minimum string length.
 * @param {string}
 *            s the string to pad.
 ...
 *        <tt>strftime</tt> function in C. The following conversion
 ...
 * 
 * </ul>
 * The following conversion specifications are currently <i>unsupported</i> for
 * formatting:
 * <ul>
 * 
  ...
  function format(d) {
    return pattern.replace(/%[a-zA-Z0-9]/g, function(s) {
        switch (s) {
          ...
          // TODO %V: week number (monday first day) [01,53] ... with
			// weirdness
          case '%w': return d.getDay();
          // TODO %W: week number (monday first day) [00,53] ... with
			// weirdness
 ...
 *        representing a duration in milliseconds, and a <tt>string</tt>. Two
 *        types of time formats are supported: "short" and "long". The <i>short</i>
 *        format type returns a string such as "3.3 days" or "12.1 minutes",
 *        while the <i>long</i> format returns "13:04:12" or similar.
 * 
 * @extends pv.Format
 * @param {string}
 *            type the type; "short" or "long".
 */
pv.Format.time = function(type) {
  ...
  /*
	 ...
	 * WEEKS = 6048e5 MONTHS = 2592e6 YEARS = 31536e6
	 */
  ...
  /**
	 ...
	 * @param {number}
	 *            t the duration in milliseconds. May also be a <tt>Date</tt>.
	 * @returns {string} the formatted string.
	 */
  format.format = format;
 ...
 * 
 * @class Represents a number format, converting between a <tt>number</tt> and
 *        a <tt>string</tt>. This class allows numbers to be formatted with
 *        variable precision (both for the integral and fractional part of the
 *        number), optional thousands grouping, and optional padding. The
 *        thousands (",") and decimal (".") separator can be customized.
 * 
...
pv.Format.number = function() {
  ...
  /**
	 ...
	 * part is truncated per the maximum integer digits, and the fraction part
	 * is rounded per the maximum fraction digits.
	 * 
	 * @function
  ...
  /**
	 ...
	 * smaller than the minimum, the digits are padded; if the number of digits
	 * is larger, the digits are truncated, showing only the lower-order digits.
	 ...
	 * @param {number}
	 *            [min] the minimum integer digits.
	 * @param {number}
	 *            [max] the maximum integer digits.
	 * @returns {pv.Format.number} <tt>this</tt>, or the current integer
	 *          digits.
	 */
  ...
  /**
	 ...
	 * separator for the fractional part of the number. If the number of digits
	 * is smaller than the minimum, the digits are padded; if the number of
	 * digits is larger, the fractional part is rounded, showing only the
	 * higher-order digits. The default range is [0, 0].
	 * 
	 * <p>
	 * If only one argument is specified to this method, this value is used as
	 * both the minimum and maximum number. If no arguments are specified, a
	 ...
	 * @param {number}
	 *            [min] the minimum fraction digits.
	 * @param {number}
	 *            [max] the maximum fraction digits.
	 * @returns {pv.Format.number} <tt>this</tt>, or the current fraction
	 *          digits.
	 */
  ...
  /**
	 * Sets or gets the character used to pad the integer part. The integer pad
	 * is used when the number of integer digits is smaller than the minimum.
	 * The default pad character is "0" (zero).
	 * 
	 * @param {string}
	 *            [x] the new pad character.
  ...
  /**
	 ...
	 * is used when the number of fraction digits is smaller than the minimum.
	 * The default pad character is "0" (zero).
	 * 
	 * @param {string}
	 *            [x] the new pad character.
  ...
  /**
	 ...
	 * 
	 * @param {string}
	 *            [x] the new decimal separator.
	 * @returns {pv.Format.number} <tt>this</tt> or the current decimal
	 *          separator.
	 */
  ...
  /**
	 ...
	 * 
	 * @param {string}
	 *            [x] the new group separator.
	 * @returns {pv.Format.number} <tt>this</tt> or the current group
	 *          separator.
	 */
  ...
  /**
	 * Sets or gets the negative prefix and suffix. The default negative prefix
	 * is "&minus;", and the default negative suffix is the empty string.
	 * 
	 * @param {string}
	 *            [x] the negative prefix.
	 * @param {string}
	 *            [y] the negative suffix.
	 * @returns {pv.Format.number} <tt>this</tt> or the current negative
	 *          format.
	 */
 ...
 *            a an array.
 * @param {number}
 *            [n] the number of times to repeat; defaults to two.
 ...
 * pairs of elements [a<sub>i</sub>, b<sub>j</sub>]. The outer loop is on
 * array <i>a</i>, while the inner loop is on <i>b</i>, such that the order of
 * returned elements is [a<sub>0</sub>, b<sub>0</sub>], [a<sub>0</sub>, b<sub>1</sub>],
 * ... [a<sub>0</sub>, b<sub>m</sub>], [a<sub>1</sub>, b<sub>0</sub>],
 * [a<sub>1</sub>, b<sub>1</sub>], ... [a<sub>1</sub>, b<sub>m</sub>],
 * ... [a<sub>n</sub>, b<sub>m</sub>]. If either array is empty, an empty
 * array is returned.
 * 
 * @param {array}
 *            a an array.
 ...
 * is to use JavaScript's <tt>concat</tt> method directly. These two
 ...
 * type="text/css">sub{line-height:0}</style> transposes each element array<sub>ij</sub>
 * with array<sub>ji</sub>. If the array has dimensions <i>n</i>&times;<i>m</i>,
 * it will have dimensions <i>m</i>&times;<i>n</i> after this method returns.
 * This method transposes the elements of the array in place, mutating the
 ...
 * numbers, an optional accessor function <tt>f</tt> can be specified to map
 * the elements to numbers. For example, if <tt>array</tt> is an array of
 * objects, and each object has a numeric property "foo", the expression
 * 
 * <pre>
 ...
 * 
 * @param {array}
 *            array an array of objects, or numbers.
 * @param {function}
 *            [f] an optional accessor function.
 ...
 * <tt>array</tt> for each index in <tt>indexes</tt>, in order. For
 * example,
 * 
 * <pre>
 ...
 * </pre>
 * 
 * returns <tt>["b", "c", "a"]</tt>. It is acceptable for the array of
 * indexes to be a different length from the array of elements, and for indexes
 * to be duplicated or omitted. The optional accessor function <tt>f</tt> can
 * be used to perform a simultaneous mapping of the array elements. Accessor
 * functions can refer to <tt>this.index</tt>.
 * 
 * @param {array}
 *            array an array.
 * @param {number[]}
 *            indexes an array of indexes into <tt>array</tt>.
 * @param {function}
 *            [f] an optional accessor function.
 ...
 * support string keys, <tt>keys</tt> must contain strings, or other values
 * that naturally map to distinct string values. Alternatively, an optional
 * accessor function <tt>f</tt> can be specified to compute the string key for
 * the given element. Accessor functions can refer to <tt>this.index</tt>.
 * 
 * @param {array}
 *            keys an array, usually of string keys.
 * @param {function}
 *            [f] an optional key function.
 ...
 * Note that since JavaScript maps only support string keys, <tt>array</tt>
 * must contain strings, or other values that naturally map to distinct string
 ...
 * 
 * @param {array}
 *            array an array, usually of string keys.
 * @param {function}
 *            [f] an optional key function.
...
/**
 * The comparator function for natural order. This can be used in conjunction
 * with the built-in array <tt>sort</tt> method to sort elements by their
 * natural order, ascending. Note that if no comparator function is specified to
 * the built-in <tt>sort</tt> method, the default order is lexicographic,
 * <i>not</i> natural!
 * 
 * @see <a
 ...
 * conjunction with the built-in array <tt>sort</tt> method to sort elements
 * by their natural order, descending. Note that if no comparator function is
 ...
 * 
 * <p>
 * The <i>insertion point</i> is defined as the point at which the value would
 * be inserted into the array: the index of the first element greater than the
 * value, or <tt>array.length</tt>, if all elements in the array are less
 ...
 * 
 * <p>
 * The method is modeled after the built-in <tt>range</tt> method from Python.
 * See the Python documentation for more details.
 * 
 * @see <a href="http://docs.python.org/library/functions.html#range">Python
 *      range</a>
 * @param {number}
...
/**
 * Returns a random number in the range [<tt>start</tt>, <tt>stop</tt>)
 * that is a multiple of <tt>step</tt>. More specifically, the returned
 * number is of the form <tt>start</tt> + <i>n</i> * <tt>step</tt>, where
 * <i>n</i> is a nonnegative integer. If <tt>step</tt> is not specified, it
 * defaults to 1, returning a random integer if <tt>start</tt> is also an
 * integer.
 * 
 * @param {number}
 *            [start] the start value value.
 * @param {number}
 *            stop the stop value.
 * @param {number}
 *            [step] the step value.
 ...
 * numbers, an optional accessor function <tt>f</tt> can be specified to map
 * the elements to numbers. See {@link #normalize} for an example. Accessor
 * functions can refer to <tt>this.index</tt>.
 * 
 * @param {array}
 *            array an array of objects, or numbers.
 ...
 * 
 * @param {number}
 *            x the number for which to compute the logarithm floor.
 * @param {number}
 *            b the base of the logarithm.
 ...
 * 
 * @param {number}
 *            x the number for which to compute the logarithm ceiling.
 * @param {number}
 *            b the base of the logarithm.
 * @returns {number} the rounded-by-logarithm value.
 */
 ...
 * The order of the returned array is not defined. Each key-value pair is
 ...
 * function <tt>f</tt> to compute the value for each key. The single argument
 * to the value function is the key. The callback is invoked only for indexes of
 ...
 * factory method, equivalent to <tt>new pv.Dom(map)</tt>. To apply the
 * operator and retrieve the root node, call {@link pv.Dom#root}; to retrieve
 * all nodes flattened, use {@link pv.Dom#nodes}.
 * 
 * @see pv.Dom
 * @param map
 ...
 *        transformation of a hierarchical JavaScript object (such as a JSON
 *        map) to a W3C Document Object Model hierarchy. For more information on
 *        which attributes and methods from the specification are supported, see
 *        {@link pv.Dom.Node}.
 * 
 * <p>
 * Leaves in the map are determined using an associated <i>leaf</i> function;
 ...
 * @param {string}
 *            [nodeName] optional node name for the root.
 */
 ...
 * 
 * @throws Error
 *             if the specified child is not a child of this node.
...
/**
 * Inserts the specified child <i>n</i> before the given reference child <i>r</i>
 * of this node. If <i>r</i> is null, this method is equivalent to
 * {@link #appendChild}. If <i>n</i> is already part of the DOM, it is first
 * removed before being inserted.
 * 
 * @throws Error
 *             if <i>r</i> is non-null and not a child of this node.
...
/**
 * Replaces the specified child <i>r</i> of this node with the node <i>n</i>.
 * If <i>n</i> is already part of the DOM, it is first removed before being
 * added.
 * 
 * @throws Error
 *             if <i>r</i> is not a child of this node.
 */
 ...
 * function <i>f</i>. The arguments to the function are:
 * <ol>
 * 
 * <li>The current node.
 * <li>The current depth, starting at 0 for the root node.
 * </ol>
 * 
 * @param {function}
 *            f a function to apply to each node.
 */
 ...
 * 
 * <p>
 * Note: during the sort operation, the comparator function should not rely on
 * the tree being well-formed; the values of <tt>previousSibling</tt> and
 * <tt>nextSibling</tt> for the nodes being compared are not defined during
 * the sort operation.
 * 
 * @param {function}
 *            f a comparator function.
 * @returns this.
 */
 ...
 * 
 * @param {boolean}
 *            [recursive] whether the toggle should apply to descendants.
 */
 ...
 * @see pv.Tree
 * @param {array}
 *            array an array from which to construct a tree.
 ...
 *        allows a hierarchical map to be constructed from an array; it is
 *        similar to the {@link pv.Nest} operator, except the hierarchy is
 *        derived dynamically from the array elements.
 * 
 * <p>
 ...
 * @see pv.Nest
 * @param {array}
 *            array an array of elements to nest.
 ...
 *        structure. The levels in the tree are specified by <i>key</i>
 *        functions. The leaf nodes of the tree can be sorted by value, while
 *        the internal nodes can be sorted by key. Finally, the tree can be
 *        returned either has a multidimensional array via {@link #entries}, or
 *        as a hierarchical map via {@link #map}. The {@link #rollup} routine
 *        similarly returns a map, collapsing the elements in each leaf node
 ...
 * 
 * @param {function}
 *            key a key function; must return a string or suitable map key.
 ...
 * <tt>order</tt> should be a comparator function. If this method is not
 * called (i.e., keys are <i>unsorted</i>), keys will appear in the order they
 * appear in the underlying elements array. For example,
 * 
 * <pre>
 * pv.nest(yields)
 *     .key(function(d) d.year)
 ...
 * 
 * @param {function}
 *            [order] an optional comparator function.
 * @returns {pv.Nest} this.
 */
 ...
 * </pre>
 * 
 * returns a map <tt>m</tt> such that <tt>m[variety][site]</tt> is an array,
 * a subset of <tt>yields</tt>, with each element having the given variety
 * and site.
 * 
 * @returns a hierarchical map of values.
 */
 ...
 * than returning a map like {@link #map}, this method returns a nested array.
 * Each element of the array has a <tt>key</tt> and <tt>values</tt> field.
 * For leaf nodes, the <tt>values</tt> array will be a subset of the
 * underlying elements array; for non-leaf nodes, the <tt>values</tt> array
 * will contain more key-values pairs.
 * 
 * <p>
...
/**
 * Returns a rollup map. The behavior of this method is the same as {@link #map},
 * except that the leaf values are replaced with the return value of the
 * specified rollup function <tt>f</tt>. For example,
 * 
 * <pre>
 * pv.nest(yields)
 *      .key(function(d) d.site)
 ...
 *        allows hierarchical maps to be flattened into an array. The levels in
 *        the input tree are specified by <i>key</i> functions.
 * 
 * <p>
 ...
 * 
 * <pre>
 * var array = pv.flatten(yields).key(&quot;year&quot;).key(&quot;variety&quot;).key(&quot;site&quot;).key(
 * 		&quot;yield&quot;).array();
 * </pre>
 * 
 ...
 *            key the key name.
 * @param {function}
 *            [f] an optional value map function.
 * @returns {pv.Nest} this.
 */
 ...
 * will be stored in the entries <tt>keys</tt> attribute. The leaf function
 * must return true for leaves, and false for internal nodes.
 * 
 * @param {function}
 *            f a leaf function.
 * @returns {pv.Nest} this.
 ...
 * 
 * @class Represents a two-dimensional vector; a 2-tuple <i>&#x27e8;x, y&#x27e9;</i>.
 *        The intent of this class is to simplify vector math. Note that in
 *        performance-sensitive cases it may be more efficient to represent 2D
 *        vectors as simple objects with <tt>x</tt> and <tt>y</tt>
 ...
 * 
 * @param {number}
 *            x the <i>x</i> coordinate to add.
 * @param {number}
 *            y the <i>y</i> coordinate to add.
 ...
 * 
 * @param {number}
 *            x the <i>x</i> coordinate to subtract.
 * @param {number}
 *            y the <i>y</i> coordinate to subtract.
 * @returns {pv.Vector} a new vector.
 */
 ...
 * 
 * @param {number}
 *            x the <i>x</i> coordinate to dot.
 * @param {number}
 *            y the <i>y</i> coordinate to dot.
 ...
 *        limited to expressing translate and uniform scale transforms only;
 *        shearing, rotation, general affine, and other transforms are not
 *        supported.
 * 
 * <p>
 * The methods on this class treat the transform as immutable, returning a copy
 * of the transformation matrix with the specified transform applied. Note,
 ...
 *        data domain to visual range. For quantitative and quantile scales, the
 *        domain is expressed as numbers; for ordinal scales, the domain is
 *        expressed as strings (or equivalently objects with unique string
 *        representations). The "visual range" may correspond to pixel space,
 *        colors, font sizes, and the like.
 * 
 * <p>
 * Note that scales are functions, and thus can be used as properties directly,
 * assuming that the data associated with a mark is a number. While this is
 * convenient for single-use scales, frequently it is desirable to define scales
 * globally:
 * 
 * <pre>
 ...
 * be passed to the <tt>y</tt> scale (e.g., <tt>d.foo</tt>). The
 * {@link #by} method similarly allows the data to be mapped to a numeric value
 ...
 * Given a scale <tt>y</tt>, <tt>y.by(function(d) d.foo)</tt> is equivalent
 * to <tt>function(d) y(d.foo)</tt>.
 * 
 * <p>
 * This method is provided for convenience, such that scales can be succinctly
 * defined inline. For example, given an array of data elements that have a
 * <tt>score</tt> attribute with the domain [0, 1], the height property could
 * be specified as:
 * 
 * <pre>
 ...
 * 
 * <p>
 * This constructor is typically not used directly; see one of the quantitative
 * scale implementations instead.
 * 
 * @class Represents an abstract quantitative scale; a function that performs a
 *        numeric transformation. This class is typically not used directly; see
 *        one of the quantitative scale implementations (linear, log, root,
 *        etc.) instead. <style type="text/css">sub{line-height:0}</style> A
 *        quantitative scale represents a 1-dimensional transformation from a
 *        numeric domain of input data [<i>d<sub>0</sub></i>, <i>d<sub>1</sub></i>]
 *        to a numeric range of pixels [<i>r<sub>0</sub></i>, <i>r<sub>1</sub></i>].
 *        In addition to readability, scales offer several useful features:
 * 
 * <p>
 ...
 * 
 * <p>
 * 2. The domain and range can be subdivided for a non-uniform transformation.
 * For example, you may want a diverging color scale that is increasingly red
 * for negative values, and increasingly green for positive values:
 * 
 * <pre>
 ...
 * 
 * <p>
 * 3. Quantitative scales can be inverted for interaction. The {@link #invert}
 ...
 * 
 * <p>
 * 5. A scale can be "niced" to extend the domain to suitable rounded numbers.
 * If the minimum and maximum of the domain are messy because they are derived
 * from data, you can use {@link #nice} to round these values down and up to
 * even numbers.
 * 
 * @param {number...}
 *            domain... optional domain values.
 * @see pv.Scale.linear
 * @see pv.Scale.log
 * @see pv.Scale.root
 * @extends pv.Scale
...
pv.Scale.quantitative = function() {
  ...
  /**
	 ...
	 * 
	 * <p>
	 * 1. <tt>domain(min, ..., max)</tt>
	 * 
	 * <p>
	 * Specifying the domain as a series of numbers is the most explicit and
	 * recommended approach. Most commonly, two numbers are specified: the
	 * minimum and maximum value. However, for a diverging scale, or other
	 * subdivided non-uniform scales, multiple values can be specified. Values
	 * can be derived from data using {@link pv.min} and {@link pv.max}. For
	 * example:
	 * 
	 * <pre>
	 ...
	 * 
	 * <p>
	 * 2. <tt>domain(array, minf, maxf)</tt>
	 * 
	 * <p>
	 * When both the minimum and maximum value are derived from data, the
	 * arguments to the <tt>domain</tt> method can be specified as the array
	 * of data, followed by zero, one or two accessor functions. For example, if
	 * the array of data is just an array of numbers:
	 * 
	 * <pre>
	 *     .domain(array)
	 * </pre>
	 ...
	 * 
	 * <p>
	 * Invoking the <tt>domain</tt> method with no arguments returns the
  ...
  /**
	 ...
	 * 
	 * <p>
	 * 1. <tt>range(min, ..., max)</tt>
	 * 
	 * <p>
	 * The range may be specified as a series of numbers or colors. Most
	 * commonly, two numbers are specified: the minimum and maximum pixel
	 * values. For a color scale, values may be specified as {@link pv.Color}s
	 * or equivalent strings. For a diverging scale, or other subdivided
	 * non-uniform scales, multiple values can be specified. For example:
	 * 
	 * <pre>
	 ...
	 * 
	 * <p>
	 * Invoking the <tt>range</tt> method with no arguments returns the
	 * current range as an array of numbers or colors.
	 * 
	 * @function
  ...
  /**
	 ...
	 * corresponding value in the input domain. This is frequently used to
	 * convert the mouse location (see {@link pv.Mark#mouse}) to a value in the
	 * input domain. Inversion is only supported for numeric ranges, and not
	 * colors.
	 * 
	 * <p>
	 * Note that this method does not do any rounding or bounds checking. If the
	 * input domain is discrete (e.g., an array index), the returned value
	 * should be rounded. If the specified <tt>y</tt> value is outside the
	 * range, the returned value may be equivalently outside the input domain.
	 * 
	 * @function
	 * @name pv.Scale.quantitative.prototype.invert
	 * @param {number}
	 *            y a value in the output range (a pixel location).
  ...
  /**
	 ...
	 * domain. This method attempts to return between 5 and 10 tick values.
	 * These values are frequently used in conjunction with {@link pv.Rule} to
	 * display tick marks or grid lines.
	 * 
	 * @function
	 * @name pv.Scale.quantitative.prototype.ticks
	 * @param {number}
	 *            [m] optional number of desired ticks.
  ...
  /**
	 * Formats the specified tick value using the appropriate precision, based
	 * on the step interval between tick marks. If {@link #ticks} has not been
	 * called, the argument is converted to a string, but no formatting is
	 * applied.
	 * 
	 * @function
  ...
  /**
	 ...
	 * dynamically from data, and may be irregular. For example, given a domain
	 * of [0.20147987687960267, 0.996679553296417], a call to <tt>nice()</tt>
  ...
  /**
	 * Returns a view of this scale by the specified accessor function
	 * <tt>f</tt>. Given a scale <tt>y</tt>,
	 * <tt>y.by(function(d) d.foo)</tt> is equivalent to
 ...
 * constructor are optional, and equivalent to calling {@link #domain}. The
 * default domain and range are [0,1].
 * 
 * @class Represents a linear scale; a function that performs a linear
 *        transformation. <style type="text/css">sub{line-height:0}</style>
 *        Most commonly, a linear scale represents a 1-dimensional linear
 *        transformation from a numeric domain of input data [<i>d<sub>0</sub></i>,
 ...
 * constructor are optional, and equivalent to calling {@link #domain}. The
 * default domain is [1,10] and the default range is [0,1].
 * 
 * @class Represents a log scale. <style type="text/css">sub{line-height:0}</style>
 *        Most commonly, a log scale represents a 1-dimensional log
 *        transformation from a numeric domain of input data [<i>d<sub>0</sub></i>,
 *        <i>d<sub>1</sub></i>] to a numeric range of pixels [<i>r<sub>0</sub></i>,
 *        <i>r<sub>1</sub></i>]. The equation for such a scale is:
 * 
 * <blockquote><i>f(x) = (log(x) - log(d<sub>0</sub>)) / (log(d<sub>1</sub>) -
 * log(d<sub>0</sub>)) * (r<sub>1</sub> - r<sub>0</sub>) + r<sub>0</sub></i></blockquote>
 * 
 * where <i>log(x)</i> represents the zero-symmetric logarthim of <i>x</i>
 * using the scale's associated base (default: 10, see {@link pv.logSymmetric}).
 * For example, a log scale from the domain [1, 100] to range [0, 640]:
 * 
 * <blockquote><i>f(x) = (log(x) - log(1)) / (log(100) - log(1)) * (640 - 0) +
 * 0</i><br>
...
pv.Scale.log = function() {
  ...
  /**
	 * Formats the specified tick value using the appropriate precision,
	 * assuming base 10.
	 * 
	 * @function
  ...
  /**
	 ...
	 * {@link pv.logCeil}. Nicing is useful if the domain is computed
	 * dynamically from data, and may be irregular. For example, given a domain
	 * of [0.20147987687960267, 0.996679553296417], a call to <tt>nice()</tt>
	 * might extend the domain to [0.1, 1].
	 * 
	 * <p>
 ...
 *        transformation. <style type="text/css">sub{line-height:0}</style>
 *        Most commonly, a root scale represents a 1-dimensional root
 *        transformation from a numeric domain of input data [<i>d<sub>0</sub></i>,
 *        <i>d<sub>1</sub></i>] to a numeric range of pixels [<i>r<sub>0</sub></i>,
 *        <i>r<sub>1</sub></i>].
 * 
 ...
 * be passed to the <tt>y</tt> scale (e.g., <tt>d.foo</tt>). The
 * {@link #by} method similarly allows the data to be mapped to a numeric value
 * before performing the root transformation.
 * 
 * @param {number...}
 *            domain... optional domain values.
 * @extends pv.Scale.quantitative
 ...
 * 
 * @class Represents an ordinal scale. <style type="text/css">sub{line-height:0}</style>
 *        An ordinal scale represents a pairwise mapping from <i>n</i> discrete
 *        values in the input domain to <i>n</i> discrete values in the output
 *        range. For example, an ordinal scale might map a domain of species
 *        ["setosa", "versicolor", "virginica"] to colors ["red", "green",
 *        "blue"]. Thus, saying
 * 
 * <pre>
 ...
 * <tt>states</tt> is an array of the fifty U.S. state names, the state name
 * can be encoded in the left position:
 * 
 * <pre>
 ...
 * 
 * <p>
 * N.B.: ordinal scales are not invertible (at least not yet), since the domain
 * and range and discontinuous. A workaround is to use a linear scale.
 * 
 * @param {...}
 *            domain... optional domain values.
 * @extends pv.Scale
...
pv.Scale.ordinal = function() {
  ...
  /**
	 ...
	 * 
	 * <p>
	 * 1. <tt>domain(values...)</tt>
	 * 
	 * <p>
	 * Specifying the domain as a series of values is the most explicit and
	 * recommended approach. However, if the domain values are derived from
	 * data, you may find the second method more appropriate.
	 * 
	 * <p>
	 * 2. <tt>domain(array, f)</tt>
	 * 
	 * <p>
	 * Rather than enumerating the domain values as explicit arguments to this
	 * method, you can specify a single argument of an array. In addition, you
	 * can specify an optional accessor function to extract the domain values
	 * from the array.
	 * 
	 * <p>
	 * 3. <tt>domain()</tt>
  ...
  /**
	 ...
	 * 
	 * <p>
	 * 2. <tt>range(array, f)</tt>
	 * 
	 * <p>
	 * Rather than enumerating the range values as explicit arguments to this
	 * method, you can specify a single argument of an array. In addition, you
	 * can specify an optional accessor function to extract the range values
	 * from the array.
	 * 
	 * <p>
	 * 3. <tt>range()</tt>
  ...
  /**
	 * Sets the range from the given continuous interval. The interval [<i>min</i>,
	 * <i>max</i>] is subdivided into <i>n</i> equispaced points, where <i>n</i>
	 * is the number of (unique) values in the domain. The first and last point
	 * are offset from the edge of the range by half the distance between
	 * points.
	 * 
	 * <p>
	 * This method must be called <i>after</i> the domain is set.
	 * 
	 * @function
	 * @name pv.Scale.ordinal.prototype.split
	 * @param {number}
	 *            min minimum value of the output range.
	 * @param {number}
	 *            max maximum value of the output range.
  ...
  /**
	 ...
	 * <i>max</i>] is subdivided into <i>n</i> equispaced points, where <i>n</i>
	 * is the number of (unique) values in the domain. The first and last point
	 * are exactly on the edge of the range.
	 * 
	 * <p>
	 * This method must be called <i>after</i> the domain is set.
	 * 
  ...
  /**
	 * Sets the range from the given continuous interval. The interval [<i>min</i>,
	 * <i>max</i>] is subdivided into <i>n</i> equispaced bands, where <i>n</i>
	 * is the number of (unique) values in the domain. The first and last band
	 * are offset from the edge of the range by the distance between bands.
	 * 
	 * <p>
	 * The band width argument, <tt>band</tt>, is typically in the range [0,
	 * 1] and defaults to 1. This fraction corresponds to the amount of space in
	 * the range to allocate to the bands, as opposed to padding. A value of 0.5
	 * means that the band width will be equal to the padding width. The
	 * computed absolute band width can be retrieved from the range as
	 * <tt>scale.range().band</tt>.
	 * 
	 * <p>
	 * If the band width argument is negative, this method will allocate bands
	 * of a <i>fixed</i> width <tt>-band</tt>, rather than a relative
	 * fraction of the available space.
	 * 
	 * <p>
	 * Tip: to inset the bands by a fixed amount <tt>p</tt>, specify a
	 * minimum value of <tt>min + p</tt> (or simply <tt>p</tt>, if
	 * <tt>min</tt> is 0). Then set the mark width to
	 * <tt>scale.range().band - p</tt>.
	 * 
	 * <p>
	 * This method must be called <i>after</i> the domain is set.
	 * 
	 ...
	 *            max maximum value of the output range.
	 * @param {number}
	 *            [band] the fractional band width in [0, 1]; defaults to 1.
	 * @returns {pv.Scale.ordinal} <tt>this</tt>.
	 * @see #split
	 */
  ...
  /**
	 ...
	 * <tt>f</tt>. Given a scale <tt>y</tt>,
	 * <tt>y.by(function(d) d.foo)</tt> is equivalent to
	 * <tt>function(d) y(d.foo)</tt>. This method should be used judiciously;
	 * it is typically more clear to invoke the scale directly, passing in the
	 * value to be scaled.
	 * 
	 * @function
	 ...
	 * @param {function}
	 *            f an accessor function.
	 * @returns {pv.Scale.ordinal} a view of this scale by the specified
	 *          accessor function.
	 */
  scale.by = function(f) {
    function by() { return scale(f.apply(this, arguments)); }
 ...
 *        a sortable domain to a quantized numeric range. Typically, the domain
 *        is a set of numbers, but any sortable value (such as strings) can be
 *        used as the domain of a quantile scale. The range defaults to [0,1],
 *        with 0 corresponding to the smallest value in the domain, 1 the
 *        largest, .5 the median, etc.
 * 
 * <p>
 * By default, the number of quantiles in the range corresponds to the number of
 * values in the domain. The {@link #quantiles} method can be used to specify an
 * explicit number of quantiles; for example, <tt>quantiles(4)</tt> produces a
 * standard quartile scale. A quartile scale's range is a set of four discrete
...
pv.Scale.quantile = function() {
  ...
  /**
	 ...
	 * domain is in its own quantile. If the argument to this method is a
	 * number, it specifies the number of equal-sized quantiles by which to
	 * divide the domain.
	 * 
	 * <p>
 ...
 * 
 * @param {array}
 *            data an array of numbers or objects.
 * @param {function}
 *            [f] an optional accessor function.
 */
pv.histogram = function(data, f) {
  ...
  return {
	 ...
	 * 
	 * <p>
	 * The returned array contains {@link pv.histogram.Bin}s. The <tt>x</tt>
 ...
 *        are themselves arrays containing the data elements present in the
 *        given bin (prior to the accessor function being invoked to convert the
 *        data object to a numeric value). These bin arrays have additional
 *        attributes with meta information about the bin.
 * 
 ...
 * 
 * </ul>
 * The SVG 1.0 color keywords names are also supported, such as "aliceblue" and
 * "yellowgreen". The "transparent" keyword is supported for fully- transparent
 * black.
 * 
 * <p>
 * If the <tt>format</tt> argument is already an instance of <tt>Color</tt>,
 * the argument is returned with no further processing.
 * 
 * @param {string}
 *            format the color specification string, such as "#f00".
 ...
 *        divided into two parts: the <tt>color</tt> attribute, an opaque
 *        color format string, and the <tt>opacity</tt> attribute, a float in
 *        [0, 1]. The color space is dependent on the implementing class; all
 *        colors support the {@link #rgb} method to convert to RGB color space
 *        for interpolation.
 * 
 * <p>
 * See also the <a href="../../api/Color.html">Color guide</a>.
 * 
 * @param {string}
 *            color an opaque color format string, such as "#f00".
 * @param {number}
...
pv.Color = function(color, opacity) {
  /**
	 ...
	 * 
	 * @type string
	 * @see <a href="http://www.w3.org/TR/SVG/types.html#ColorKeywords">SVG
	 *      color keywords</a>
	 * @see <a href="http://www.w3.org/TR/css3-color/">CSS3 color module</a>
	 */
  this.color = color;
 ...
 * 
 * @param {number}
 *            r the red channel, an integer in [0,255].
 * @param {number}
 *            g the green channel, an integer in [0,255].
 * @param {number}
 *            b the blue channel, an integer in [0,255].
 * @param {number}
 *            [a] the alpha channel, a float in [0,1].
 ...
 * 
 * @param {number}
 *            h the hue, an integer in [0, 360].
 * @param {number}
 *            s the saturation, a float in [0, 1].
 * @param {number}
 *            l the lightness, a float in [0, 1].
 * @param {number}
 *            [a] the opacity, a float in [0, 1].
 ...
 * optional, and equivalent to calling {@link pv.Scale.OrdinalScale#domain}.
 * The following colors are used:
 * 
 * <div style="background:#1f77b4;">#1f77b4</div> <div
 * style="background:#ff7f0e;">#ff7f0e</div> <div
 * style="background:#2ca02c;">#2ca02c</div> <div
 * style="background:#d62728;">#d62728</div> <div
 * style="background:#9467bd;">#9467bd</div> <div
 * style="background:#8c564b;">#8c564b</div> <div
 * style="background:#e377c2;">#e377c2</div> <div
 * style="background:#7f7f7f;">#7f7f7f</div> <div
 * style="background:#bcbd22;">#bcbd22</div> <div
 * style="background:#17becf;">#17becf</div>
 * 
 * @param {number...}
 *            domain... domain values.
 ...
 * 
 * <div style="background:#1f77b4;">#1f77b4</div> <div
 * style="background:#aec7e8;">#aec7e8</div> <div
 * style="background:#ff7f0e;">#ff7f0e</div> <div
 * style="background:#ffbb78;">#ffbb78</div> <div
 * style="background:#2ca02c;">#2ca02c</div> <div
 * style="background:#98df8a;">#98df8a</div> <div
 * style="background:#d62728;">#d62728</div> <div
 * style="background:#ff9896;">#ff9896</div> <div
 * style="background:#9467bd;">#9467bd</div> <div
 * style="background:#c5b0d5;">#c5b0d5</div> <div
 * style="background:#8c564b;">#8c564b</div> <div
 * style="background:#c49c94;">#c49c94</div> <div
 * style="background:#e377c2;">#e377c2</div> <div
 * style="background:#f7b6d2;">#f7b6d2</div> <div
 * style="background:#7f7f7f;">#7f7f7f</div> <div
 * style="background:#c7c7c7;">#c7c7c7</div> <div
 * style="background:#bcbd22;">#bcbd22</div> <div
 * style="background:#dbdb8d;">#dbdb8d</div> <div
 * style="background:#17becf;">#17becf</div> <div
 * style="background:#9edae5;">#9edae5</div>
 * 
 * @param {number...}
 *            domain... domain values.
 * @returns {pv.Scale.ordinal} a new ordinal color scale.
 ...
 * are optional, and equivalent to calling {@link pv.Scale.OrdinalScale#domain}.
 * The following colors are used:
 * 
 * <div style="background:#9c9ede;">#9c9ede</div> <div
 * style="background:#7375b5;">#7375b5</div> <div
 * style="background:#4a5584;">#4a5584</div> <div
 * style="background:#cedb9c;">#cedb9c</div> <div
 * style="background:#b5cf6b;">#b5cf6b</div> <div
 * style="background:#8ca252;">#8ca252</div> <div
 * style="background:#637939;">#637939</div> <div
 * style="background:#e7cb94;">#e7cb94</div> <div
 * style="background:#e7ba52;">#e7ba52</div> <div
 * style="background:#bd9e39;">#bd9e39</div> <div
 * style="background:#8c6d31;">#8c6d31</div> <div
 * style="background:#e7969c;">#e7969c</div> <div
 * style="background:#d6616b;">#d6616b</div> <div
 * style="background:#ad494a;">#ad494a</div> <div
 * style="background:#843c39;">#843c39</div> <div
 * style="background:#de9ed6;">#de9ed6</div> <div
 * style="background:#ce6dbd;">#ce6dbd</div> <div
 * style="background:#a55194;">#a55194</div> <div
 * style="background:#7b4173;">#7b4173</div>
 * 
 * @param {number...}
 *            domain... domain values.
 * @returns {pv.Scale.ordinal} a new ordinal color scale.
...
pv.Colors.category19 = function() {
  var scale = pv.colors(
      "#969696", "#222222", "#333333", "#444444", "#555555",
      "#666666", "#777777", "#888888", "#999999", "#aaaaaa",
      "#bbbbbb", "#cccccc", "#969696", "#969696");
  scale.domain.apply(scale, arguments);
  return scale;
};
/**
 * Returns a linear color ramp from the specified <tt>start</tt> color to the
 * specified <tt>end</tt> color. The color arguments may be specified either
 * as <tt>string</tt>s or as {@link pv.Color}s. This is equivalent to:
 * 
 * <pre>
 ...
 * 
 * @param {string}
 *            start the start color; may be a <tt>pv.Color</tt>.
 * @param {string}
 *            end the end color; may be a <tt>pv.Color</tt>.
 ...
 * 
 * @param type
 *            {string} an SVG element type, such as "rect".
 ...
 *            {string} an SVG element type, such as "rect".
 * @param attributes
 *            an optional attribute map.
 * @param style
 *            an optional style map.
 * @returns a new SVG element.
 */
 ...
 * <tt>title</tt> property of the specified scene node <tt>s</tt>. Note
 * that this implementation does not create an SVG <tt>title</tt> element as a
 * child of <tt>e</tt>; although this is the recommended standard, it is only
...
pv.SvgScene.pathBasis = (function() {
  ...
  /**
	 * Returns the point that is the weighted sum of the specified control
	 * points, using the specified weights. This method requires that there are
	 * four weights and four control points.
	 */
...
/**
 * @private Interpolates the given points using the basis spline interpolation.
 *          If points.length == tangents.length then a regular Hermite
 *          interpolation is performed, if points.length == tangents.length + 2
 *          then the first and last segments are filled in with cubic bazier
 *          segments. Returns an array of path strings.
 * 
 * @param points
 *            the array of points.
 */
pv.SvgScene.curveBasisSegments = function(points) {
  ...
  p2 = p3;
  p3 = points[2];
  paths.push(firstPath + this.pathBasis(p0, p1, p2, p3)); // merge first &
															// second path
 ...
 *          cubic Hermite spline interpolation. If points.length ==
 *          tangents.length then a regular Hermite interpolation is performed,
 *          if points.length == tangents.length + 2 then the first and last
 *          segments are filled in with cubic bazier segments. Returns an SVG
 *          path without the leading M instruction to allow path appending.
 * 
 * @param points
 *            the array of points.
 * @param tangents
...
/**
 * @private Interpolates the given points with respective tangents using the
 *          cubic Hermite spline interpolation. Returns an array of path
 *          strings.
 * 
 * @param points
 *            the array of points.
 ...
 *          spline interpolation. Returns an array of tangent vectors. Note:
 *          that for n points only the n-2 well defined tangents are returned.
 * 
 * @param points
 *            the array of points.
 * @param tension
...
/**
 * @private Interpolates the given points using Fritsch-Carlson Monotone cubic
 *          Hermite interpolation. Returns an SVG path without the leading M
 *          instruction to allow path appending.
 * 
 * @param points
 *            the array of points.
 */
...
/**
 * @private Interpolates the given points using Fritsch-Carlson Monotone cubic
 *          Hermite interpolation. Returns an array of path strings.
 * 
 * @param points
 *            the array of points.
 */
 ...
 *        the base class for all graphical marks in Protovis; it does not
 *        provide any specific rendering functionality, but together with
 *        {@link Panel} establishes the core framework.
 * 
 * <p>
 * Concrete mark types include familiar visual elements such as bars, lines and
 * labels. Although a bar mark may be used to construct a bar chart, marks know
 * nothing about charts; it is only through their specification and composition
 * that charts are produced. These building blocks permit many combinatorial
 * possibilities.
 * 
 * <p>
 ...
 * <ol>
 * 
 * <li>If the <tt>width</tt> property is not specified (i.e., null), its
 * value is the width of the parent panel, minus this mark's left and right
 * margins; the left and right margins are zero if not specified.
 * 
 ...
 * 
 * <li>Otherwise, if the <tt>left</tt> property is not specified, its value
 * is the width of the parent panel, minus this mark's width and the right
 * margin.
 * 
 * </ol>
 * This prioritization is then duplicated for the <tt>height</tt>,
 ...
 * 
 * <p>
 * See also the <a href="../../api/">Protovis guide</a>.
 */
pv.Mark = function() {
  /*
	 * TYPE 0 constant defs TYPE 1 function defs TYPE 2 constant properties TYPE
	 * 3 function properties in order of evaluation!
	 */
  this.$properties = [];
  this.$handlers = {};
};
 ...
 *          given name. This method should be called on a mark class prototype
 *          to define each exposed property. (Note this refers to the JavaScript
 *          <tt>prototype</tt>, not the Protovis mark prototype, which is the
 *          {@link #proto} field.)
 * 
 * <p>
 ...
 * <ol>
 * 
 * <li>If invoked with a <tt>Function</tt> argument, this function is
 * evaluated for each associated datum. The return value of the function is used
 * as the computed property value. The context of the function (<tt>this</tt>)
 * is this mark. The arguments to the function are the associated data of this
 * mark and any enclosing panels. For example, a linear encoding of numerical
 ...
 * </pre>
 * 
 * The expression <tt>d * 100</tt> will be evaluated for the height property
 * of each mark instance. The return value of the property method (e.g.,
 * <tt>m.height</tt>) is this mark (<tt>m</tt>)).
 * <p>
 * 
 ...
 * <p>
 * 
 * <li>If invoked with no arguments, the computed property value for the
 * current mark instance in the scene graph is returned. This facilitates
 * <i>property chaining</i>, where one mark's properties are defined in terms
 * of another's. For example, to offset a mark's location from its prototype,
 * you might say
 * 
 * <pre>
 ...
 * <tt>this.proto</tt>) is inherited from the <tt>Mark</tt> class and set
 * by this mark. So, if the fifth element's top property is being evaluated, the
 * fifth instance of <tt>this.proto</tt> will similarly be queried for the
 * value of its top property. If the mark being evaluated has a different number
 * of instances, or its data is unrelated, the behavior of this method is
 * undefined. In these cases it may be better to index the <tt>scene</tt>
 * explicitly to specify the exact instance.
 * 
 * </ol>
 * <p>
 * Property names should follow standard JavaScript method naming conventions,
 * using lowerCamel-style capitalization.
 * 
 * <p>
 * In addition to creating the property method, every property is registered in
 * the {@link #properties} map on the <tt>prototype</tt>. Although this is an
 * instance field, it is considered immutable and shared by all instances of a
 * given mark type. The <tt>properties</tt> map can be queried to see if a
 * mark type defines a particular property, such as width or height.
 * 
 * @param {string}
 *            name the property name.
 * @param {function}
 *            [cast] the cast function for this property.
 */
pv.Mark.prototype.property = function(name, cast) {
  ...
  /*
	 * Define the setter-getter globally, since the default behavior should be
	 * the same for all properties, and since the Protovis inheritance chain is
 ...
 *            name the property name.
 * @param {boolean}
 *            [def] whether is a property or a def.
 * @param {function}
 *            [cast] the cast function for this property.
 */
 ...
 *          (or "node") corresponds to an instance of this mark and an element
 *          in the data array. The scene graph can be traversed to lookup
 ...
 * 
 * @param {function}
 *            type the type of mark to add; a constructor, such as
 *            <tt>pv.Bar</tt>.
 ...
 * @param {function}
 *            [v] an optional initializer; may be a constant or a function.
 */
 ...
 * 
 * </ul>
 * In addition to positioning properties (left, right, top bottom), the anchors
 * support text rendering properties (text-align, text-baseline). Text is
 * rendered to appear inside the mark by default.
 * 
 * <p>
 * To facilitate stacking, anchors are defined in terms of their opposite edge.
 * For example, the top anchor defines the bottom property, such that the mark
 * extends upwards; the bottom anchor instead defines the top property, such
 * that the mark extends downwards. See also {@link pv.Layout.Stack}.
 * 
 * <p>
 ...
 * 
 * @param {string}
 *            name the anchor name; either a string or a property function.
 ...
 *          is typically equivalent to <tt>this.scene[this.index]</tt>,
 *          however if the scene or index is unset, the default instance of the
 *          mark is returned. If no default is set, the default is the last
 *          instance. Similarly, if the scene or index of the parent panel is
 *          unset, the default instance of this mark in the last instance of the
 *          enclosing panel is returned, and so on.
 * 
...
pv.Mark.prototype.instances = function(source) {
  ...
  /*
	 * When the anchor target is also an ancestor, as in the case of adding to a
	 * panel anchor, only generate one instance per panel. Also, set the margins
	 * to zero, since they are offset by the enclosing panel.
	 */
 ...
 *          method can only be called when the mark is bound to the scene graph
 *          (for example, from an event handler, or within a property function).
 * 
 * @returns a node in the scene graph.
 */
 ...
 *          method can only be called when the mark is bound to the scene graph
 *          (for example, from an event handler, or within a property function).
 *          In addition, note that mark instances are built sequentially, so the
 *          last instance of this mark may not yet be constructed.
 * 
 * @returns a node in the scene graph.
 */
 ...
 *          previous instance of the enclosing parent panel. May return null if
 *          this instance could not be found.
 * 
 * @returns a node in the scene graph, or null.
 */
 ...
 * 
 * <p>
 * If an enclosing panel has an index property set (as is the case inside in an
 * event handler), then only instances of this mark inside the given instance of
 * the panel will be rendered; otherwise, all visible instances of the mark will
 * be rendered.
 */
pv.Mark.prototype.render = function() {
  ...
  function render(mark, depth, scale) {
    ...
    if (depth < indexes.length) {
      ...
      /*
		 ...
		 * during the update phase; instead the values computed previously in
		 * the build phase are simply translated into SVG. The update phase is
  ...
  /**
	 ...
	 *          This is slightly tricky because `index` and `scene` properties
	 *          may or may not already be set; if they are set, it means we are
	 *          rendering only a specific instance; if they are unset, we are
	 *          rendering all instances. Furthermore, we must preserve the
	 *          original context of these properties when rendering completes.
	 * 
	 * <p>
	 * Another tricky aspect is that the `scene` attribute should be set for any
	 * preceding children, so as to allow property chaining. This is consistent
	 * with first-pass rendering.
	 */
 ...
 * 
 * <p>
 * The evaluation of the <tt>data</tt> and <tt>visible</tt> properties is
 ...
 * mark, since the data is not defined until the data property is evaluated. The
 * <tt>visisble</tt> property is subsequently evaluated for each instance;
 ...
 * 
 * <p>
 * If this mark is being re-built, any old instances of this mark that no longer
 * exist (because the new data array contains fewer elements) will be cleared
 * using {@link #clearInstance}.
 * 
 * @param parent
 ...
 *          instance <tt>s</tt> in the scene graph. The set of properties to
 *          evaluate is retrieved from the {@link #properties} array for this
 *          mark type (see {@link #type}). After these properties are evaluated,
 *          any <b>implied</b> properties may be computed by the mark and set
 *          on the scene graph; see {@link #buildImplied}.
 * 
 * <p>
 ...
 *          instance <tt>s</tt> in the scene graph. Implied properties are
 *          those with dependencies on multiple other properties; for example,
 *          the width property may be implied if the left and right properties
 *          are set. This method can be overridden by concrete mark types to
 *          define new implied properties, if necessary.
 * 
 * @param s
 *            a node in the scene graph; the instance of the mark to build.
 */
 ...
 * 
 * <p>
 * The complete set of event types is defined by SVG; see the reference below.
 * The set of supported event types is:
 * <ul>
 * 
 * <li>click
 * <li>mousedown
 ...
 * 
 * </ul>
 * Since Protovis does not specify any concept of focus, it does not support key
 * events; these should be handled outside the visualization using standard
 * JavaScript. In the future, support for interaction may be extended to support
 * additional event types, particularly those most relevant to interactive
 * visualization, such as selection.
 * 
 * <p>
 ...
 * be used.
 * 
 * @see <a href="http://www.w3.org/TR/SVGTiny12/interact.html#SVGEvents">SVG
 *      events</a>
 * @param {string}
 *            type the event type.
 * @param {function}
 ...
 *        without a visual representation. It serves only to provide useful
 *        default properties that can be inherited by other marks. Each type of
 *        mark can define any number of named anchors for convenience. If the
 *        concrete mark type does not define an anchor implementation
 *        specifically, one will be inherited from the mark's parent class.
 * 
 * <p>
 * For example, the bar mark provides anchors for its four sides: left, right,
 * top and bottom. Adding a label to the top anchor of a bar,
 * 
 * <pre>
 ...
 * 
 * <p>
 * Note that anchors do not <i>inherit</i> from their targets; the positional
 ...
 * 
 * <p>
 * This method differs slightly from the normal mark behavior in that the
 ...
 *        connected line segments. Unsurprisingly, areas are used most
 *        frequently for area charts.
 * 
 * <p>
 * Just as a line represents a polyline, the <tt>Area</tt> mark type
 * represents a <i>polygon</i>. However, an area is not an arbitrary polygon;
 * vertices are paired either horizontally or vertically into parallel <i>spans</i>,
 * and each span corresponds to an associated datum. Either the width or the
 * height must be specified, but not both; this determines whether the area is
 * horizontally-oriented or vertically-oriented. Like lines, areas can be
 * stroked and filled with arbitrary colors.
 * 
 * <p>
 * See also the <a href="../../api/Area.html">Area guide</a>.
 * 
 * @extends pv.Mark
 */
pv.Area = function() {
 ...
 * 
 * <p>
 * This property is <i>fixed</i> for non-segmented areas. See {@link pv.Mark}.
 * 
 * @type number
 ...
 * 
 * <p>
 * This property is <i>fixed</i>. See {@link pv.Mark}.
 * 
 * @type boolean
 ...
 *          evaluated, even if the first segment is not visible. Also cache
 *          which properties are normally fixed.
 */
 ...
 *          determined dynamically, based on the value of the (always) fixed
 *          segmented property. Any fixed properties are only evaluated on the
 *          first instance, although their values are propagated to subsequent
 *          instances, so that they are available for property chaining and the
 *          like.
 */
 ...
 * In addition to positioning properties (left, right, top bottom), the anchors
 * support text rendering properties (text-align, text-baseline). Text is
 * rendered to appear inside the area. The area anchor also propagates the
 ...
 * 
 * <p>
 * For consistency with the other mark types, the anchor positions are defined
 * in terms of their opposite edge. For example, the top anchor defines the
 * bottom property, such that an area added to the top anchor grows upward.
 * 
 * @param {string}
 *            name the anchor name; either a string or a property function.
 * @returns {pv.Anchor}
 ...
 *        filled. Bars are used for many chart types, including bar charts,
 *        histograms and Gantt charts. Bars can also be used as decorations, for
 *        example to draw a frame border around a panel; in fact, a panel is a
 *        special type (a subclass) of bar.
 * 
 * <p>
 ...
 * 
 * <p>
 * See also the <a href="../../api/Bar.html">Bar guide</a>.
 * 
 * @extends pv.Mark
 */
pv.Bar = function() {
 ...
 *        point that can also be stroked and filled. The <tt>size</tt>
 *        property is proportional to the area of the rendered glyph to
 *        encourage meaningful visual encodings. Dots can visually encode up to
 *        eight dimensions of data, though this may be unwise due to
 *        integrality. See {@link pv.Mark} for details on the prioritization of
 *        redundant positioning properties.
 * 
 * <p>
 * See also the <a href="../../api/Dot.html">Dot guide</a>.
 * 
 * @extends pv.Mark
 */
pv.Dot = function() {
 ...
 * 
 * </ul>
 * These shapes can be further changed using the {@link #angle} property; for
 * instance, a cross can be turned into a plus by rotating. Similarly, the tick,
 * which is vertical by default, can be rotated horizontally. Note that some
 * shapes (cross and tick) do not have interior areas, and thus do not support
 * fill style meaningfully.
 * 
 * <p>
 * Note: it may be more natural to use the {@link pv.Rule} mark for horizontal
 * and vertical ticks. The tick shape is only necessary if angled ticks are
 * needed.
 * 
 * @type string
 ...
 * For consistency with the other mark types, the anchor positions are defined
 * in terms of their opposite edge. For example, the top anchor defines the
 * bottom property, such that a bar added to the top anchor grows upward.
 * 
 * @param {string}
 *            name the anchor name; either a string or a property function.
 * @returns {pv.Anchor}
 ...
 *        arbitrary text within the visualization. The character data must be
 *        plain text (unicode), though the text can be styled using the
 *        {@link #font} property. If rich text is needed, external HTML elements
 *        can be overlaid on the canvas by hand.
 * 
 * <p>
 * Labels are positioned using the box model, similarly to {@link Dot}. Thus, a
 * label has no width or height, but merely a text anchor location. The text is
 * positioned relative to this anchor location based on the {@link #textAlign},
 * {@link #textBaseline} and {@link #textMargin} properties. Furthermore, the
 * text may be rotated using {@link #textAngle}.
 * 
 * <p>
 ...
 * 
 * <p>
 * See also the <a href="../../api/Label.html">Label guide</a>.
 * 
 * @extends pv.Mark
 */
pv.Label = function() {
 ...
 * in a direction dependent on the horizontal and vertical alignment properties.
 * For example, if the text is left- and middle-aligned, the margin shifts the
 * text to the right. The default margin is 3 pixels.
 * 
 * @type number
 ...
 *        articulation point in the line corresponds to a datum; for <i>n</i>
 *        points, <i>n</i>-1 connected line segments are drawn. The point is
 *        positioned using the box model. Arbitrary paths are also possible,
 *        allowing radar plots and other custom visualizations.
 * 
 * <p>
 ...
 * 
 * <p>
 * See also the <a href="../../api/Line.html">Line guide</a>.
 * 
 * @extends pv.Mark
 */
pv.Line = function() {
 ...
 *        for axes and grid lines. For example, specifying only the bottom
 *        property draws horizontal rules, while specifying only the left draws
 *        vertical rules. Rules can also be used as thin bars. The visual style
 *        is controlled in the same manner as lines.
 * 
 * <p>
 ...
 * 
 * <table> <thead>
 * <th style="width:12em;">Properties</th>
 * <th>Orientation</th>
 * </thead> <tbody>
 * <tr>
 * <td>left</td>
 ...
 * </tr>
 * <tr>
 * <td>left, bottom, top</td>
 * <td>vertical</td>
 * </tr>
 * <tr>
 * <td>right, bottom, top</td>
 * <td>vertical</td>
 * </tr>
 * <tr>
 * <td>top</td>
 ...
 * </tr>
 * <tr>
 * <td>bottom, left, right</td>
 * <td>horizontal</td>
 * </tr>
 * <tr>
 * <td>left, top, height</td>
 * <td>vertical</td>
 * </tr>
 * <tr>
 * <td>left, bottom, height</td>
 * <td>vertical</td>
 * </tr>
 * <tr>
 * <td>right, top, height</td>
 * <td>vertical</td>
 * </tr>
 * <tr>
 * <td>right, bottom, height</td>
 * <td>vertical</td>
 * </tr>
 * <tr>
 * <td>left, top, width</td>
 * <td>horizontal</td>
 * </tr>
 * <tr>
 * <td>left, bottom, width</td>
 * <td>horizontal</td>
 * </tr>
 * <tr>
 * <td>right, top, width</td>
 * <td>horizontal</td>
 * </tr>
 * <tr>
 * <td>right, bottom, width</td>
 * <td>horizontal</td>
 * </tr>
 * </tbody> </table>
 * 
 * <p>
 * Small rules can be used as tick marks; alternatively, a {@link Dot} with the
 * "tick" shape can be used.
 * 
 * <p>
 * See also the <a href="../../api/Rule.html">Rule guide</a>.
 * 
 * @see pv.Line
 * @extends pv.Mark
 */
 ...
 *        dimensions. Other types of visualizations may benefit from repeated
 *        and possibly overlapping structure as well, such as stacked area
 *        charts. Panels can also offset the position of marks to provide
 *        padding from surrounding content.
 * 
 * <p>
 ...
 * 
 * <p>
 * Panels can be rendered inline, facilitating the creation of sparklines. This
 * allows designers to reuse browser layout features, such as text flow and
 * tables; designers can also overlay HTML elements such as rich text and
 * images.
 * 
 * <p>
 * All panels have a <tt>children</tt> array (possibly empty) containing the
...
pv.Panel = function() {
  ...
  /**
	 * The internal $dom field is set by the Protovis loader; see lang/init.js.
	 * It refers to the script element that contains the Protovis specification,
	 * so that the panel knows where in the DOM to insert the generated SVG
	 * element.
	 * 
	 * @private
	 */
 ...
 * 
 * <p>
 * Note: the "canvas" element here refers to a <tt>div</tt> (or other suitable
...
/**
 * Specifies whether child marks are clipped when they overflow this panel. This
 * affects the clipping of all this panel's descendant marks.
 * 
 * @type string
 ...
 * 
 * @param {function}
 *            type the type of the new mark to add.
 * @returns {pv.Mark} the new mark.
 */
 ...
 *          instance <tt>s</tt> in the scene graph, including recursively
 *          building the scene graph for child marks.
 * 
 * @param s
 *            a node in the scene graph; the instance of the panel to build.
 * @see Mark#scene
 */
pv.Panel.prototype.buildInstance = function(s) {
  ...
  /*
	 * Build each child, passing in the parent (this panel) scene graph node.
	 * The child mark's scene is initialized from the corresponding entry in the
  ...
  /*
	 * Once the child marks have been built, the new scene graph nodes are
	 * removed from the child marks and placed into the scene graph. The nodes
	 * cannot remain on the child nodes because this panel (or a parent panel)
 ...
 *          <ul>
 * 
 * <li>The <tt>canvas</tt> property references the DOM element, typically a
 * DIV, that contains the SVG element that is used to display the visualization.
 * This property may be specified as a string, referring to the unique ID of the
 ...
 * 
 * </ul>
 * The current implementation creates the SVG element, if necessary, during the
 * build phase; in the future, it may be preferrable to move this to the update
 * phase, although then the canvas property would be undefined. In addition, DOM
 * inspection is necessary to define the implied width and height properties
 * that may be inferred from the DOM.
 * 
 * @param s
 *            a node in the scene graph; the instance of the panel to build.
 */
 ...
 *        generated pixel buffer. Images share the same layout and style
 *        properties as bars. The external image resource is specified via the
 *        {@link #url} property. The optional fill, if specified, appears
 ...
 * psuedo-property. This function is passed the <i>x</i> and <i>y</i> index,
 * in addition to the current data stack. The return value is a {@link pv.Color},
 ...
 * <tt>r</tt>, <tt>g</tt>, <tt>b</tt> and <tt>a</tt> attributes, to
 * avoid the cost of parsing and object instantiation.
 * 
 * <p>
 ...
 * computed implicitly from the loaded image resources. For dynamic images, this
 * property can be used to specify the height of the pixel buffer; otherwise,
 * the value is derived from the <tt>height</tt> property.
 * 
 * @type number
 ...
 * 
 * <p>
 * For example, given a two-dimensional array <tt>heatmap</tt>, containing
 ...
 * palette, or return an object literal with <tt>r</tt>, <tt>g</tt>,
 * <tt>b</tt> and <tt>a</tt> attributes. A {@link pv.Color} or string can
 * also be returned, though this typically results in slower performance.
 * 
 * @param {function}
 *            f the new sizing function.
 ...
 *        angle, inner and outer radius, wedges can be used to construct donut
 *        charts and polar bar charts as well. If the {@link #angle} property is
 *        used, the end angle is implied by adding this value to start angle. By
 *        default, the start angle is the previously-generated wedge's end
 *        angle. This design allows explicit control over the wedge placement if
 *        desired, while offering convenient defaults for the construction of
 *        radial graphs.
 * 
 * <p>
 ...
 * 
 * <p>
 * See also the <a href="../../api/Wedge.html">Wedge guide</a>.
 * 
 * @extends pv.Mark
 */
pv.Wedge = function() {
 ...
 * the end angle of the previous instance (the {@link Mark#sibling}), or -PI /
 * 2 for the first wedge; for pie and donut charts, typically only the
 ...
 * </ul>
 * In addition to positioning properties (left, right, top bottom), the anchors
 * support text rendering properties (text-align, text-baseline, textAngle).
 * Text is rendered to appear inside the wedge.
 * 
 * @param {string}
 *            name the anchor name; either a string or a property function.
 * @returns {pv.Anchor}
 ...
 *        two-dimensional space. Forces can be applied to these particles,
 *        causing them to move. Constraints can also be applied to restrict
 *        particle movement, for example, constraining particles to a fixed
 *        position, or simulating collision between circular particles with
 *        area.
 * 
 * <p>
 ...
 * href="http://www.teknikus.dk/tj/gdc2001.htm">geometric constraints</a> can
 * be implemented. For each time step, Verlet integration is performed, new
 * forces are accumulated, and then constraints are applied.
 * 
 * <p>
 ...
 * 
 * @param {array}
 *            particles an array of {@link pv.Particle}s to simulate.
 * @see pv.Layout.Force
 * @see pv.Force
 * @see pv.Constraint
 */
...
pv.Simulation.prototype.step = function() {
  ...
  /*
	 * Assumptions: - The mass (m) of every particles is 1. - The time step (dt)
	 * is 1.
	 */
 ...
 *        subdivision. This particular implementation uses square partitions,
 *        dividing each square into four equally-sized squares. Each particle
 *        exists in a unique node; if multiple particles are in the same
 *        position, some particles may be stored on internal nodes rather than
 *        leaf nodes.
 * 
 * <p>
 * This quadtree can be used to accelerate various spatial operations, such as
 * the Barnes-Hut approximation for computing n-body forces, or collision
 * detection.
 * 
 * @see pv.Force.charge
 * @see pv.Constraint.collision
 * @param {pv.Particle}
 *            particles the linked list of particles.
 */
 ...
 *        does not specify how to bind a force to specific particles; in
 *        general, forces are applied globally to all particles. However, some
 *        forces may be applied to specific particles or between particles, such
 *        as spring forces, through additional specialization.
 * 
 * @see pv.Simulation
 * @see pv.Particle
 * @see pv.Force.charge
 ...
 * @param {pv.Particle}
 *            particles particles to which to apply this force.
 * @param {pv.Quadtree}
 *            q a quadtree for spatial acceleration.
 */
/**
 ...
 *        gravitation, inversely proportional to the square of the distance
 *        between particles. Note that the force is independent of the <i>mass</i>
 *        of the associated particles, and that the particles do not have
 *        charges of varying magnitude; instead, the attraction or repulsion of
 *        all particles is globally specified as the charge {@link #constant}.
 * 
 * <p>
 ...
 * hierarchical O(N log N) force-calculation algorithm"</a>, J. Barnes &amp; P.
 * Hut, <i>Nature</i> 1986.
 * 
 * @name pv.Force.charge
 * @param {number}
 *            [k] the charge constant.
 */
pv.Force.charge = function(k) {
  ...
  /**
	 ...
	 * applying forces that are two strong (due to granularity of the
	 * simulation's numeric integration). A maximum distance threshold improves
	 * performance by skipping force calculations for particles that are far
	 * apart.
	 * 
	 * <p>
  ...
  /**
	 ...
	 * @param {number}
	 *            x the new Barnes-Hut approximation factor.
	 * @returns {pv.Force.charge} this.
	 */
  ...
  /**
	 ...
	 *         quadtree. This is equivalent to the center of mass, assuming that
	 *         all particles have unit weight.
	 */
  ...
  /**
	 ...
	 *         quadtree node. The Barnes-Hut approximation criterion is the
	 *         ratio of the size of the quadtree node to the distance from the
	 *         point to the node's center of mass is beneath some threshold.
	 */
  ...
  /**
	 * Applies this force to the specified particles. The force is applied
	 * between all pairs of particles within the domain, using the specified
	 * quadtree to accelerate n-body force calculation using the Barnes-Hut
 ...
 *        applied in the opposite direction of the particle's velocity. Since
 *        Position Verlet integration does not track velocities explicitly, the
 *        error term with this estimate of velocity is fairly high, so the drag
 ...
 *        configured with a tension constant, rest length, and damping factor.
 *        The tension and damping will automatically be normalized using the
 *        inverse square root of the maximum link degree of attached nodes; this
 *        makes springs weaker between nodes of high link degree.
 * 
 * <p>
...
pv.Force.spring = function(k) {
  ...
  /**
	 * Sets or gets the spring constant. The default value is 0.1; greater
	 * values will result in stronger tension. The spring tension is
	 * automatically normalized using the inverse square root of the maximum
	 * link degree of attached nodes.
	 * 
	 * @function
	 * @name pv.Force.spring.prototype.constant
	 * @param {number}
	 *            x the new spring constant.
  ...
  /**
	 ...
	 * in the opposite direction of attached nodes' velocities. The default
	 * value is 0.1. The spring damping is automatically normalized using the
	 * inverse square root of the maximum link degree of attached nodes.
	 * 
	 * @function
	 * @name pv.Force.spring.prototype.damping
	 * @param {number}
	 *            x the new spring damping factor.
 ...
 *        interface does not specify how to bind a constraint to specific
 *        particles; in general, constraints are applied globally to all
 *        particles. However, some constraints may be applied to specific
 *        particles or between particles, such as position constraints, through
 ...
 * @param {pv.Particle}
 *            particles particles to which to apply this constraint.
 * @param {pv.Quadtree}
 *            q a quadtree for spatial acceleration.
 ...
 *        circle, with the radius of the particle computed using a specified
 *        function. For example, if the particle has an <tt>r</tt> attribute
 *        storing the radius, the radius <tt>function(d) d.r</tt> specifies a
 *        collision constraint using this radius. The radius function is passed
 *        each {@link pv.Particle} as the first argument.
 * 
 * <p>
 * To accelerate collision detection, this implementation uses a quadtree and a
 * search radius. The search radius is computed as the maximum radius of all
...
pv.Constraint.collision = function(radius) {
  ...
  /**
	 ...
	 * method for constraints relaxation. Repeating the collision constraint
	 * makes the constraint have more of an effect when there is a potential for
	 * many co-occurring collisions.
	 * 
	 * @function
	 * @name pv.Constraint.collision.prototype.repeat
	 * @param {number}
	 *            x the number of times to repeat this constraint.
 ...
 *        particle is determined using a given position function, which defaults
 *        to <tt>function(d) d.fix</tt>.
 * 
 * <p>
 * If the position function returns null, then no position constraint is applied
 * to the given particle. Otherwise, the particle's position is set to the
 * returned position, as expressed by a {@link pv.Vector}. (Note: the position
 * does not need to be an instance of <tt>pv.Vector</tt>, but simply an
 * object with <tt>x</tt> and <tt>y</tt> attributes.)
 * 
 * <p>
 * This constraint also supports a configurable alpha parameter, which defaults
 * to 1. If the alpha parameter is in the range [0,1], then rather than setting
 * the particle's new position directly to the position returned by the supplied
 * position function, the particle's position is interpolated towards the fixed
 * position. This results is a smooth (exponential) drift towards the fixed
 * position, which can increase the stability of the physics simulation. In
 * addition, the alpha parameter can be decayed over time, relaxing the position
 * constraint, which helps to stabilize on an optimal solution.
 * 
 * @param {function}
...
pv.Constraint.position = function(f) {
  ...
  /**
	 ...
	 * @param {number}
	 *            x the new alpha parameter, in the range [0,1].
 ...
 *        this constraint can be used to constrain particles in a physics
 *        simulation within the bounds of an enclosing panel.
 * 
 * <p>
...
pv.Constraint.bound = function() {
  ...
  /**
	 ...
	 * @param {number}
	 *            min the minimum allowed x-coordinate.
	 * @param {number}
	 *            max the maximum allowed x-coordinate.
  ...
  /**
	 ...
	 * @param {number}
	 *            min the minimum allowed y-coordinate.
	 * @param {number}
	 *            max the maximum allowed y-coordinate.
	 * @returns {pv.Constraint.bound} this.
	 */
 ...
 *        extending from {@link pv.Panel}, and defining a set of mark
 *        prototypes as children. These mark prototypes provide default
 *        properties that together implement the given visualization technique.
 * 
 * <p>
 * Layouts do not initially contain any marks; any exported marks (such as a
 * network layout's <tt>link</tt> and <tt>node</tt>) are intended to be
 * used as prototypes. By adding a concrete mark, such as a {@link pv.Bar}, to
 * the appropriate mark prototype, the mark is added to the layout and inherits
 * the given properties. This approach allows further customization of the
 * layout, either by choosing a different mark type to add, or more simply by
 * overriding some of the layout's defined properties.
 * 
 * <p>
 ...
 * 
 * <p>
 * In addition to the mark prototypes, layouts may define custom properties that
 * affect the overall behavior of the layout. For example, a treemap layout
 ...
 *          although the property method is only defined locally, the cast
 *          function is global, which is necessary since properties are
 *          inherited!
 * 
 * @param {string}
 *            name the property name.
 ...
 *        force-directed graph layout) and space-filling network diagrams (such
 *        as sunbursts and treemaps). Note that "network" here is a general term
 *        that includes hierarchical structures; a tree is represented using
 ...
 * <ul>
 * 
 * <li><tt>nodes</tt> - an array of objects representing nodes. Objects in
 * this array must conform to the {@link pv.Layout.Network.Node} interface;
 * which is to say, be careful to avoid naming collisions with automatic
 * attributes such as <tt>index</tt> and <tt>linkDegree</tt>. If the nodes
 * property is defined as an array of primitives, such as numbers or strings,
 * these primitives are automatically wrapped in an object; the resulting
 * object's <tt>nodeValue</tt> attribute points to the original primitive
 * value.
 * 
 * <p>
 * <li><tt>links</tt> - an array of objects representing links. Objects in
 ...
 * <tt>sourceNode</tt> and <tt>targetNode</tt> references must be set. Note
 * that if the links property is defined after the nodes property, the links can
 * be defined in terms of <tt>this.nodes()</tt>.
 * 
 * </ul>
 * 
 * <p>
 ...
 * <ul>
 * 
 * <li><tt>node</tt> - for rendering nodes; typically a {@link pv.Dot}. The
 * node mark is added directly to the layout, with the data property defined via
 * the layout's <tt>nodes</tt> property. Properties such as
 * <tt>strokeStyle</tt> and <tt>fillStyle</tt> can be overridden to compute
 * properties from node data dynamically.
 * 
 * <p>
 * <li><tt>link</tt> - for rendering links; typically a {@link pv.Line}. The
 ...
 * 
 * <p>
 * <li><tt>label</tt> - for rendering node labels; typically a
 ...
 * 
 * </ul>
 * Note that some network implementations may not support all three standard
 * mark prototypes; for example, space-filling hierarchical layouts typically do
 * not use a <tt>link</tt> prototype, as the parent-child links are implied by
 * the structure of the space-filling <tt>node</tt> marks. Check the specific
 * network layout for implementation details.
 * 
 * <p>
 * Network layout properties, including <tt>nodes</tt> and <tt>links</tt>,
 * are typically cached rather than re-evaluated with every call to render. This
 * is a performance optimization, as network layout algorithms can be expensive.
 * If the network structure changes, call {@link #reset} to clear the cache
 * before rendering. Note that although the network layout properties are
...
pv.Layout.Network = function() {
  ...
  /**
	 * The node label prototype, which renders the node name adjacent to the
	 * node. This prototype is provided as an alternative to using the anchor on
	 * the node mark; it is primarily intended to be used with radial node-link
 ...
 *        constructor; this class merely serves to document the attributes that
 *        are used on nodes in network layouts. (Note that hierarchical nodes
 *        place additional requirements on node representation, vis
 *        {@link pv.Dom.Node}.)
 * 
 ...
 *        constructor; this class merely serves to document the attributes that
 *        are used on links in network layouts. For hierarchical layouts, this
 *        class is used to represent the parent-child links.
 * 
 * @see pv.Layout.Network
...
/**
 * Alias for <tt>sourceNode</tt>, as expressed by the index of the source
 * node. This attribute is not populated automatically, but may be used as a
 * more convenient identification of the link's source, for example in a static
 * JSON representation.
 * 
 * @type number
...
/**
 * Alias for <tt>targetNode</tt>, as expressed by the index of the target
 * node. This attribute is not populated automatically, but may be used as a
 * more convenient identification of the link's target, for example in a static
 * JSON representation.
 * 
 * @type number
...
/**
 * Alias for <tt>linkValue</tt>. This attribute is not populated
 * automatically, but may be used instead of the <tt>linkValue</tt> attribute
 * when specifying links.
 * 
 * @type number
 ...
 *        specialization of {@link pv.Layout.Network}, providing the basic
 *        structure for both hierarchical node-link diagrams (such as
 *        Reingold-Tilford trees) and space-filling hierarchy diagrams (such as
 *        sunbursts and treemaps).
 * 
 * <p>
 * Unlike general network layouts, the <tt>links</tt> property need not be
 ...
 * 
 * <p>
 * Correspondingly, the <tt>nodes</tt> property is represented as a union of
 ...
 * 
 * <p>
 * For more details on how to use this layout, see {@link pv.Layout.Network}.
 * 
 ...
 * 
 * <p>
 * This layout exports a single <tt>cell</tt> mark prototype, which is
 ...
 * 
 * <pre>
 * vis.add(pv.Layout.Grid).rows(arrays).cell.add(pv.Bar).fillStyle(
 * 		pv.ramp(&quot;white&quot;, &quot;black&quot;))
 * </pre>
 * 
 ...
 *        stacked bar charts to more elaborate "streamgraphs" composed of
 *        stacked areas. Stack layouts uses length as a visual encoding, as
 *        opposed to position, as the layers do not share an aligned axis.
 * 
 * <p>
 ...
 * array of numbers. The <tt>x</tt> and <tt>y</tt> psuedo-properties are
 * used to define the thickness of each layer at the given position,
 * respectively; in the above example of the "bottom-left" orientation, the
 * <tt>x</tt> and <tt>y</tt> psuedo-properties are equivalent to the
 * <tt>left</tt> and <tt>height</tt> properties that you might use if you
 * implemented a stacked area by hand.
 * 
 * <p>
 * The advantage of using the stack layout is that the baseline, i.e., the
 * <tt>bottom</tt> property is computed automatically using the specified
 * offset algorithm. In addition, the order of layers can be computed using a
 * built-in algorithm via the <tt>order</tt> property.
 * 
 * <p>
 * With the exception of the "expand" <tt>offset</tt>, the stack layout does
 * not perform any automatic scaling of data; the values returned from
 * <tt>x</tt> and <tt>y</tt> specify pixel sizes. To simplify scaling math,
 * use this layout in conjunction with {@link pv.Scale.linear} or similar.
 * 
 * <p>
 * In other cases, the <tt>values</tt> psuedo-property can be used to define
 ...
 * <tt>causes</tt>). Additional arguments specify the data of enclosing
 * panels, if any.
 * 
 * @extends pv.Layout
 */
pv.Layout.Stack = function() {
  ...
  /**
	 ...
	 * bar or panel mark (or subclass thereof). Other mark types may be
	 * possible, though note that the stack layout is not currently designed to
	 * support radial stacked visualizations using wedges.
	 * 
	 * <p>
 ...
 * 
 * </ul>
 * For details on these offset algorithms, refer to "Stacked Graphs -- Geometry
 * &amp; Aesthetics" by L. Byron and M. Wattenberg, IEEE TVCG November/December
 * 2008.
 * 
 * @type string
 ...
 */
/**
 * Constructs a new, empty treemap layout. Layouts are not typically constructed
 * directly; instead, they are added to an existing panel via
 * {@link pv.Mark#add}.
 * 
 * @class Implements a space-filling rectangular layout, with the hierarchy
 *        represented via containment. Treemaps represent nodes as boxes, with
 *        child nodes placed within parent boxes. The size of each box is
 *        proportional to the size of the node in the tree. This particular
 *        algorithm is taken from Bruls, D.M., C. Huizing, and J.J. van Wijk, <a
 *        href="http://www.win.tue.nl/~vanwijk/stm.pdf">"Squarified Treemaps"</a>
 *        in <i>Data Visualization 2000, Proceedings of the Joint Eurographics
 *        and IEEE TCVG Sumposium on Visualization</i>, 2000, pp. 33-42.
 * 
 * <p>
 ...
 * <ul>
 * 
 * <li><tt>node</tt> - for rendering nodes; typically a {@link pv.Bar}. The
 * node data is populated with <tt>dx</tt> and <tt>dy</tt> attributes, in
 * addition to the standard <tt>x</tt> and <tt>y</tt> position attributes.
 * 
 * <p>
 * <li><tt>leaf</tt> - for rendering leaf nodes only, with no fill or stroke
 * style by default; typically a {@link pv.Panel} or another layout!
 * 
 * <p>
 * <li><tt>link</tt> - unsupported; undefined. Links are encoded implicitly
 ...
 * 
 * </ul>
 * For more details on how to use this layout, see {@link pv.Layout.Hierarchy}.
 * 
 ...
 *      >"Ordered Treemap Layouts"</a> by B. Shneiderman &amp; M. Wattenberg,
 *      IEEE InfoVis 2001.
 */

/**
 * The sibling node order. A <tt>null</tt> value means to use the sibling
 * order specified by the nodes property as-is; "reverse" will reverse the given
 ...
 * <tt>nodes</tt> property. For example, if the tree data structure represents
 * a file system, with files as leaf nodes, and each file has a <tt>bytes</tt>
 ...
 *        tree layout algorithm. The specific algorithm used by this layout is
 *        based on <a
 *        href="http://citeseer.ist.psu.edu/buchheim02improving.html">"Improving
 *        Walker's Algorithm to Run in Linear Time"</A> by C. Buchheim, M.
 *        J&uuml;nger &amp; S. Leipert, Graph Drawing 2002. This layout supports
 *        both cartesian and radial orientations orientations for node-link
 *        diagrams.
 * 
 * <p>
 * The tree layout supports a "group" property, which if true causes siblings to
 * be positioned closer together than unrelated nodes at the same depth. The
 ...
 * <li>radial - radially, with the root at the center.
 * </ul>
 * 
 * @type string
 ...
 *        preorder traversal, and nodes are indented based on their depth from
 *        the root. This technique is used ubiquitously by operating systems to
 *        represent file directories; although it requires much vertical space,
 *        indented trees allow efficient <i>interactive</i> exploration of
 *        trees to find a specific node. In addition they allow rapid scanning
 *        of node labels, and multivariate data such as file sizes can be
 *        displayed adjacent to the hierarchy.
 * 
 * <p>
 * The indent layout can be configured using the <tt>depth</tt> and
 * <tt>breadth</tt> properties, which control the increments in pixel space
 * for each indent and row in the layout. This layout does not support multiple
 * orientations; the root node is rendered in the top-left, while
 * <tt>breadth</tt> is a vertical offset from the top, and <tt>depth</tt> is
 * a horizontal offset from the left.
 * 
 * <p>
 * For more details on how to use this layout, see {@link pv.Layout.Hierarchy}.
 * 
 ...
 * 
 * </ul>
 * The pack layout support dynamic sizing for leaf nodes, if a {@link #size}
 * psuedo-property is specified. The default size function returns 1, causing
 * all leaf nodes to be sized equally, and all internal nodes to be sized by the
 * number of leaf nodes they have as descendants.
 * 
 * <p>
 * The size function can be used in conjunction with the order property, which
 * allows the nodes to the sorted by the computed size. Note: for sorting based
 * on other data attributes, simply use the default <tt>null</tt> for the
 ...
 *      >"Visualization of large hierarchical data by circle packing"</a> by W.
 *      Wang, H. Wang, G. Dai, and H. Wang, ACM CHI 2006.
 */
 ...
 * 
 * <p>
 * For example, if the tree data structure represents a file system, with files
 * as leaf nodes, and each file has a <tt>bytes</tt> attribute, you can
 ...
 *        layout uses the Fruchterman-Reingold algorithm, which applies an
 *        attractive spring force between neighboring nodes, and a repulsive
 *        electrical charge force between all nodes. An additional drag force
 *        improves stability of the simulation. See {@link pv.Force.spring},
 *        {@link pv.Force.drag} and {@link pv.Force.charge} for more details;
 *        note that the n-body charge force is approximated using the Barnes-Hut
 *        algorithm.
 * 
 * <p>
 ...
 * 
 * <p>
 * Force layouts are typically interactive. The gradual movement of the nodes as
 * they stabilize to a local stress minimum can help reveal the structure of the
 * network, as can {@link pv.Behavior.drag}, which allows the user to pick up
 * nodes and reposition them while the physics simulation continues. This layout
 * can also be used with pan &amp; zoom behaviors for interaction.
 * 
 * <p>
 * To facilitate interaction, this layout by default automatically re-renders
 * using a <tt>setInterval</tt> every 42 milliseconds. This can be disabled
 * via the <tt>iterations</tt> property, which if non-null specifies the
 * number of simulation iterations to run before the force-directed layout is
 * finalized. Be careful not to use too high an iteration count, as this can
 * lead to an annoying delay on page load.
 * 
 * <p>
 * As with other network layouts, the network data can be updated dynamically,
 * provided the property cache is reset. See {@link pv.Layout.Network} for
 * details. New nodes are initialized with random positions near the center.
 * Alternatively, positions can be specified manually by setting the <tt>x</tt>
 * and <tt>y</tt> attributes on nodes.
 * 
 * @extends pv.Layout.Network
 * @see <a
 *      href="http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.13.8444&rep=rep1&type=pdf"
 ...
 * 
 * <p>
 * The spring tension is automatically normalized using the inverse square root
 * of the maximum link degree of attached nodes.
 * 
 * @type number
 ...
 * 
 * <p>
 * The spring damping is automatically normalized using the inverse square root
 * of the maximum link degree of attached nodes.
 * 
 * @type number
...
/**
 * Constructs a new, empty cluster layout. Layouts are not typically constructed
 * directly; instead, they are added to an existing panel via
 * {@link pv.Mark#add}.
 * 
 ...
 *        {@link pv.Layout.Partition}, except that leaf nodes are positioned at
 *        maximum depth, and the depth of internal nodes is based on their
 *        distance from their deepest descendant, rather than their distance
 *        from the root.
 * 
 * <p>
 * The cluster layout supports a "group" property, which if true causes siblings
 * to be positioned closer together than unrelated nodes at the same depth.
 * Unlike the partition layout, this layout does not support dynamic sizing for
 * leaf nodes; all leaf nodes are the same size.
 * 
 * <p>
 * For more details on how to use this layout, see {@link pv.Layout.Hierarchy}.
 * 
 ...
 * 
 * </ul>
 * For more details on how to use this layout, see {@link pv.Layout.Cluster}.
 * 
 ...
 *        icicle) algorithm. This layout provides both node-link and
 *        space-filling implementations of partition diagrams. In many ways it
 *        is similar to {@link pv.Layout.Cluster}, except that leaf nodes are
 *        positioned based on their distance from the root.
 * 
 * <p>
 ...
 * 
 * </ul>
 * For more details on how to use this layout, see {@link pv.Layout.Partition}.
 * 
 ...
 *        visualization with a one-dimensional layout of nodes, using circular
 *        arcs to render links between nodes. For undirected networks, arcs are
 *        rendering on a single side; this makes arc diagrams useful as
 *        annotations to other two-dimensional network layouts, such as rollup,
 *        matrix or table layouts. For directed networks, links in opposite
 *        directions can be rendered on opposite sides using
 *        <tt>directed(true)</tt>.
 * 
 * <p>
 ...
 * 
 * <p>
 * This visualization technique is related to that developed by M. Wattenberg,
 * <a href="http://www.research.ibm.com/visual/papers/arc-diagrams.pdf">"Arc
 * Diagrams: Visualizing Structure in Strings"</a> in <i>IEEE InfoVis</i>,
 * 2002. However, this implementation is limited to simple node-link networks,
 * as opposed to structures with hierarchical self-similarity (such as strings).
 * 
 * <p>
 ...
 * 
 * </ul>
 * For more details on how this layout is structured and can be customized, see
 ...
 * 
 * <p>
 * Note that arc diagrams are particularly sensitive to order. This is referred
 * to as the seriation problem, and many different techniques exist to find good
 * node orders that emphasize clusters, such as spectral layout and simulated
 * annealing.
 * 
 * @param {function}
 *            f comparator function for nodes.
 ...
 * <tt>nodes</tt> property. The following orientations are supported:
 * <ul>
 * 
 * <li>left - left-to-right.
 * <li>right - right-to-left.
 * <li>top - top-to-bottom.
 * <li>bottom - bottom-to-top.
 * <li>radial - radially, starting at 12 o'clock and proceeding clockwise.
 * </ul>
 * 
 * @type string
 ...
 *        area chart where the area is folded into multiple bands. Color is used
 *        to encode band, allowing the size of the chart to be reduced
 *        significantly without impeding readability. This layout algorithm is
 *        based on the work of J. Heer, N. Kong and M. Agrawala in <a
 ...
 * 
 * <p>
 * This layout exports a single <tt>band</tt> mark prototype, which is
 ...
 * 
 * <p>
 * The <tt>fillStyle</tt> of the area can be overridden, though typically it
 * is easier to customize the layout's behavior through the custom
 * <tt>backgroundStyle</tt>, <tt>positiveStyle</tt> and
 * <tt>negativeStyle</tt> properties. By default, the background is white,
 * positive bands are blue, and negative bands are red. For the most accurate
 * presentation, use fully-opaque colors of equal intensity for the negative and
 * positive bands.
 * 
 * @extends pv.Layout
 */
 ...
 *        nodes are rolled up along two dimensions. This implementation is based
 *        on the "PivotGraph" designed by Martin Wattenberg:
 * 
 ...
 *      >"Visual Exploration of Multivariate Graphs"</a> by M. Wattenberg, CHI
 *      2006.
 */
 ...
 *        effect, a visualization of the graph's <i>adjacency matrix</i>: the
 *        cell at row <i>i</i>, column <i>j</i>, corresponds to the link from
 *        node <i>i</i> to node <i>j</i>. The fill color of each cell is
 *        binary by default, and corresponds to whether a link exists between
 *        the two nodes. If the underlying graph has links with variable values,
 *        the <tt>fillStyle</tt> property can be substited to use an
 *        appropriate color function, such as {@link pv.ramp}.
 * 
 * <p>
 ...
 * sides of the diagonal using <tt>directed(true)</tt>. The graph is assumed
 * to be undirected by default.
 * 
 * <p>
 * The mark prototypes for this network layout are slightly different than other
 * implementations:
 * <ul>
 * 
 ...
 * 
 * <p>
 * <li><tt>link</tt> - for rendering links; typically a {@link pv.Bar}. The
 ...
 * {@link pv.Network.Layout.Link}, though the <tt>linkValue</tt> attribute
 * may be 0 if no link exists in the graph.
 * 
 * <p>
 * <li><tt>label</tt> - for rendering node labels; typically a
 * {@link pv.Label}. The label mark is added directly to the layout, with the
 ...
 * 
 * <p>
 * Note that matrix visualizations are particularly sensitive to order. This is
 ...
 *        a given mark. Behaviors are themselves functions designed to be used
 *        as event handlers. For example, to add pan and zoom support to any
 *        panel, say:
 * 
 * <pre>
 ...
 * 
 * <p>
 * Each behavior implementation has specific requirements as to which events it
 * supports, and how it should be used. For example, the drag behavior requires
 * that the data associated with the mark be an object with <tt>x</tt> and
 * <tt>y</tt> attributes, such as a {@link pv.Vector}, storing the mark's
 ...
 *        Register this behavior on marks that should be draggable by the user,
 *        such as the selected region for brushing and linking. This behavior
 *        can be used in tandom with {@link pv.Behavior.select} to allow the
 *        selected region to be dragged interactively.
 * 
 * <p>
 ...
 * 
 * <p>
 * This behavior requires that the data associated with the mark being dragged
 * have <tt>x</tt> and <tt>y</tt> attributes that correspond to the mark's
 * location in pixels. The mark's positional properties are not set directly by
 * this behavior; instead, the positional properties should be defined as:
 * 
 * <pre>
 *     .left(function(d) d.x)
 *     .top(function(d) d.y)
 ...
 * 
 * <p>
 * The drag behavior is bounded by the parent panel; the <tt>x</tt> and
 ...
 * 
 * <p>
 * The mark being dragged is automatically re-rendered for each mouse event as
 * part of the drag operation. In addition, a <tt>fix</tt> attribute is
 ...
 * rendered. This can be accomplished by listening for the drag psuedo-events:
 * <ul>
 * 
 ...
 * 
 * </ul>
 * For example, to render the parent panel while dragging, thus re-rendering all
 * sibling marks:
 * 
 * <pre>
 ...
 *        close proximity to the mouse cursor. This behavior is an alternative
 *        to the native mouseover and mouseout events, improving usability.
 *        Rather than requiring the user to mouseover a mark exactly, the mouse
 *        simply needs to move near the given mark and a "point" event is
 *        triggered. In addition, if multiple marks overlap, the point behavior
 *        can be used to identify the mark instance closest to the cursor, as
 *        opposed to the one that is rendered on top.
 * 
 * <p>
 * The point behavior can also identify the closest mark instance for marks that
 * produce a continuous graphic primitive. The point behavior can thus be used
 * to provide details-on-demand for both discrete marks (such as dots and bars),
 * as well as continuous marks (such as lines and areas).
 * 
 * <p>
 ...
 * 
 * <p>
 * By default, the standard Cartesian distance is computed. However, with some
 * visualizations it is desirable to consider only a single dimension, such as
 * the <i>x</i>-dimension for an independent variable. In this case, the
 ...
 * 
 * <p>
 * This behavior only listens to mousemove events on the assigned panel, which
 * is typically the root panel. The behavior will search recursively for
 ...
 * set the <tt>events</tt> property to "all" such that the panel receives
 ...
 * 
 * @param {number}
 *            [r] the fuzzy radius threshold in pixels
 * @see <a href="http://www.tovigrossman.com/papers/chi2005bubblecursor.pdf"
 *      >"The Bubble Cursor: Enhancing Target Acquisition by Dynamic Resizing of
 *      the Cursor's Activation Area"</a> by T. Grossman &amp; R. Balakrishnan,
 *      CHI 2005.
 */
pv.Behavior.point = function(r) {
  ...
  /**
	 ...
	 * distance is computed. However, with some visualizations it is desirable
	 * to consider only a single dimension, such as the <i>x</i>-dimension for
	 * an independent variable. In this case, the collapse parameter can be set
	 * to collapse the <i>y</i> dimension:
	 * 
	 * <pre>
	 *     .event(&quot;mousemove&quot;, pv.Behavior.point(Infinity).collapse(&quot;y&quot;))
	 * </pre>
 ...
 *        Register this behavior on panels that should be selectable by the
 *        user, such for brushing and linking. This behavior can be used in
 *        tandom with {@link pv.Behavior.drag} to allow the selected region to
 ...
 * <p>
 * This behavior requires that the data associated with the mark being dragged
 * have <tt>x</tt>, <tt>y</tt>, <tt>dx</tt> and <tt>dy</tt> attributes
 ...
 * 
 * <p>
 * The panel being selected is automatically re-rendered for each mouse event as
 * part of the drag operation. This behavior may be enhanced in the future to
 ...
 * 
 * </ul>
 * For example, to render the parent panel while selecting, thus re-rendering
 * all sibling marks:
 * 
 * <pre>
 *     .event(&quot;mousedown&quot;, pv.Behavior.drag())
 ...
 *        events. Register this behavior on selection handles that should be
 *        resizeable by the user, such for brushing and linking. This behavior
 *        can be used in tandom with {@link pv.Behavior.select} and
 *        {@link pv.Behavior.drag} to allow the selected region to be selected
 *        and dragged interactively.
 * 
 * <p>
 * After the initial mousedown event is triggered, this behavior listens for
 ...
 * 
 * <p>
 * This behavior requires that the data associated with the mark being resized
 * have <tt>x</tt>, <tt>y</tt>, <tt>dx</tt> and <tt>dy</tt> attributes
 * that correspond to the mark's location and dimensions in pixels. The mark's
 * positional properties are not set directly by this behavior; instead, the
 * positional properties should be defined as:
 ...
 * 
 * <p>
 * The mark being resized is automatically re-rendered for each mouse event as
 * part of the resize operation. This behavior may be enhanced in the future to
 * allow more flexible configuration. In some cases, such as with parallel
 ...
 * 
 * </ul>
 * For example, to render the parent panel while resizing, thus re-rendering all
 * sibling marks:
 * 
 * <pre>
 ...
 *        Register this behavior on panels to allow panning. This behavior can
 *        be used in tandem with {@link pv.Behavior.zoom} to allow both panning
 *        and zooming:
 * 
 * <pre>
 *     .event(&quot;mousedown&quot;, pv.Behavior.pan())
 ...
 * 
 * <p>
 * The implementation of this behavior relies on the panel's <tt>transform</tt>
 * property, which specifies a matrix transformation that is applied to child
 * marks. Note that the transform property only affects the panel's children,
 * but not the panel itself; therefore the panel's fill and stroke will not
 * change when the contents are panned.
 * 
 * <p>
 * Panels have transparent fill styles by default; this means that panels may
 ...
 * 
 * <p>
 * The pan behavior has optional support for bounding. If enabled, the user will
 * not be able to pan the panel outside of the initial bounds. This feature is
 * designed to work in conjunction with the zoom behavior; otherwise, bounding
 * the panel effectively disables all panning.
 * 
 * @extends pv.Behavior
 * @see pv.Behavior.zoom
 * @see pv.Panel#transform
...
pv.Behavior.pan = function() {
  ...
  /**
	 * Sets or gets the bound parameter. If bounding is enabled, the user will
	 * not be able to pan outside the initial panel bounds; this typically
	 * applies only when the pan behavior is used in tandem with the zoom
	 * behavior. Bounding is not enabled by default.
	 * 
	 * <p>
 ...
 *        behavior on panels to allow zooming. This behavior can be used in
 *        tandem with {@link pv.Behavior.pan} to allow both panning and zooming:
 * 
 * <pre>
 *     .event(&quot;mousedown&quot;, pv.Behavior.pan())
 *     .event(&quot;mousewheel&quot;, pv.Behavior.zoom())
 ...
 * marks. Note that the transform property only affects the panel's children,
 * but not the panel itself; therefore the panel's fill and stroke will not
 * change when the contents are zoomed. The built-in support for transforms only
 * supports uniform scaling and translates, which is sufficient for panning and
 * zooming. Note that this is not a strict geometric transformation, as the
 * <tt>lineWidth</tt> property is scale-aware: strokes are drawn at constant
 * size independent of scale.
 * 
 * <p>
 * Panels have transparent fill styles by default; this means that panels may
...
pv.Behavior.zoom = function(speed) {
  ...
  /**
	 * Sets or gets the bound parameter. If bounding is enabled, the user will
	 * not be able to zoom out farther than the initial panel bounds. Bounding
	 * is not enabled by default. If this behavior is used in tandem with the
	 * pan behavior, both should use the same bound parameter.
	 * 
	 * <p>
	 * Note: enabling bounding after zooming has already occurred will not
	 * immediately reset the transform. Bounding should be enabled before the
	 * zoom behavior is applied.
	 * 
	 * @function
 ...
 *        coordinates (latitude and longitude) and normalized screen space in
 *        the range [-1,1]. The remaining mapping between normalized screen
 *        space and actual pixels is performed by <tt>pv.Geo.scale</tt>.
 * 
 * <p>
 ...
 * 
 * <p>
 * In general, this class should not be used directly, unless the desire is to
 * implement a new geographic projection. Instead, use <tt>pv.Geo.scale</tt>.
 ...
 * @param {pv.Geo.LatLng}
 *            latlng the latitude and longitude to project.
 ...
 * @param {pv.Vector}
 *            xy the x- and y-coordinates to invert.
 ...
 *        coordinates and screen pixel coordinates. By default, the domain is
 *        inferred from the geographic coordinates, so that the domain fills the
 *        output range.
 * 
 * <p>
 * Note that geographic scales are two-dimensional transformations, rather than
 * the one-dimensional bidrectional mapping typical of other scales. Rather than
 * mapping (for example) between a numeric domain and a numeric range,
 * geographic scales map between two coordinate objects: {@link pv.Geo.LatLng}
 * and {@link pv.Vector}.
 * 
 * @param {pv.Geo.Projection}
...
pv.Geo.scale = function(p) {
  ...
  /**
	 ...
	 * 
	 * @namespace Tick functions for geographic scales. Because geographic
	 *            scales represent two-dimensional transformations (as opposed
	 *            to one-dimensional transformations typical of other scales),
	 *            the tick values are similarly represented as two-dimensional
	 *            coordinates in the input domain, i.e., {@link pv.Geo.LatLng}
	 *            objects.
	 * 
	 * <p>
	 ...
	 * latitude. Therefore the returned array of ticks is a two-dimensional
	 * array, sampling various latitudes as constant longitude, and vice versa.
	 * 
	 * <p>
  ...
  /**
	 ...
	 * projected to normalized space, effectively finding the domain in
	 * normalized space rather than in terms of latitude and longitude. Thus,
	 * changing the projection requires recomputing the normalized domain.
	 * 
	 * <p>
	 ...
	 * function to extract the domain values (as {@link pv.Geo.LatLng}s) from
	 * the array. If the specified array has fewer than two elements, this scale
	 * will default to the full normalized domain.
	 * 
	 * <p>
	 * 2. <tt>domain()</tt>
	 * 
  ...
  /**
	 ...
	 * 
	 * <p>
	 * 2. <tt>range(width, height)</tt>
	 * 
	 * <p>
	 * If two numbers are specified, the arguments specify the maximum values of
	 * the x- and y-coordinates explicitly; the minimum values are implicitly
	 * zero.
	 * 
	 * <p>
	 * 3. <tt>range()</tt>
	 ...
	 * <p>
	 * Invoking the <tt>range</tt> method with no arguments returns the
	 * current range as an array of two {@link pv.Vector}s: the minimum
	 * (top-left) and maximum (bottom-right) values.
	 * 
	 * @function
  ...
  /**
	 ...
	 * 
	 * <p>
	 * 2. <tt>projection(object)</tt>
	 * 
	 * <p>
	 * Specifying an object sets the projection to the given custom projection,
	 * which must implement the <i>forward</i> and <i>inverse</i> methods per
	 * the {@link pv.Geo.Projection} interface.
	 * 
	 * <p>
	 * 3. <tt>projection()</tt>
	 * 
	 * <p>
	 * Invoking the <tt>projection</tt> method with no arguments returns the
  ...
  /**
	 * Returns a view of this scale by the specified accessor function
	 * <tt>f</tt>. Given a scale <tt>g</tt>,
	 * <tt>g.by(function(d) d.foo)</tt> is equivalent to
	 * <tt>function(d) g(d.foo)</tt>. This method should be used judiciously;
	 * it is typically more clear to invoke the scale directly, passing in the
	 * value to be scaled.
	 * 
	 * @function
